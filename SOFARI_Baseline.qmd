...
version: 1.0
date: "2025-Aug-11"
date-modified: "2025-September-06"
type: website
output-dir: docs
editor: visual
...
bibliography: SOFARI_Baseline.bib
---
...
Packages were loaded in advance for further processing.
...
Access to registry datasets was pre-authorised using the *gargle* package.
...
A series of functions were generated to call later in the file for processing data. The first function was constructed to retrieve and process input files, using the *googledrive* package.
...
The next function conducted categorisation of terms for pathology (diagnosis) stored in the registry using regular expressions.
...
The next function concatenated cleaned and replaced terms for each treatment record using *tidyverse* syntax. It included a method to cumulatively concatenate terms within a treatment record that have been split into text sequences delimited by punctuation (e.g. ";").
...
A function was created to run a regression model of the same form across all FAOS subscales in sequence.

```{r}
#|label: regression-func


# Function to run models for each subscale
run_subscale_models <- function(outcome_var, formula_str, model_label) {
  
  # Create models for this subscale
  models <- map(1:MasterImp$m, ~ {
    fixest::feols(
      as.formula(paste(outcome_var, formula_str)),
      data = complete(MasterImp, action = .x),
      cluster = ~PatientID
    )
  })
  
  # Pool parameters and add labels
  parameters::pool_parameters(models) |>
    mutate(
      Model = model_label,
      Subscale = subscale_labels[which(faos_subscales == outcome_var)]
    )
}
```

A function was created to pool coefficient estimates across multiply imputation sets after adjusted regression.
...
# Function to create pairwise comparisons from regional coefficients
create_pairwise_comparisons <- function(pooled_results) {
  
  # Extract regional coefficients (assuming reference is the first region alphabetically)
  regional_coefs <- pooled_results |>
    filter(str_detect(Parameter, "^Region")) |>
    mutate(
      Region = str_remove(Parameter, "^Region"),
      # Clean up coefficient names
      Region = str_trim(Region)
    )
  
  # Get unique regions from the data
  regions <- unique(regional_coefs$Region)
  reference_region <- setdiff(
    # Assuming you know your reference region - adjust as needed
    c("Ankle", "Foot", "Hindfoot", "Midfoot", "Multiple"), # Replace with your actual regions
    regions
  )[1]
  
  # Create all pairwise combinations
  all_regions <- c(reference_region, regions)
  pairwise_combinations <- utils::combn(all_regions, 2, simplify = FALSE)
  
  # Function to calculate pairwise difference
  calculate_pairwise_diff <- function(region1, region2, subscale_data) {
    
    # Get coefficients (reference region has coefficient = 0)
    coef1 <- if(region1 == reference_region) {
      list(Coefficient = 0, SE = 0)
    } else {
      subscale_data |> 
        filter(Region == region1) |>
        select(Coefficient, SE) |>
        as.list()
    }
    
    coef2 <- if(region2 == reference_region) {
      list(Coefficient = 0, SE = 0)
    } else {
      subscale_data |> 
    ...
    diff_estimate <- coef1$Coefficient - coef2$Coefficient
    
    # For pooled results, we approximate the SE of the difference
    # This is conservative but reasonable for presentation
    combined_se <- sqrt(coef1$SE^2 + coef2$SE^2)
    
    # Calculate t-statistic and p-value (using large sample approximation)
    t_stat <- diff_estimate / combined_se
    p_value <- 2 * pnorm(abs(t_stat), lower.tail = FALSE)
    
    tibble(
      comparison = paste(region1, "vs", region2),
  ...
  # Create pairwise comparisons for each subscale and model type
  results <- pooled_results |>
    filter(str_detect(Parameter, "^Region")) |>
    mutate(Region = str_remove(Parameter, "^Region")) |>
    select(Subscale, Model, Region, Coefficient, SE) |>
    nest(data = c(Region, Coefficient, SE)) |>
    mutate(
      pairwise_comparisons = map(data, ~ {
        subscale_data <- .x
        map_dfr(pairwise_combinations, ~ {
          calculate_pairwise_diff(.x[1], .x[2], subscale_data)
        })
      })
    ) |>
    select(-data) |>
...
Subset analysis required its own function to re-run regression models.

```{r}
# Function to run models for each subscale
run_subset_models <- function(outcome_var, formula_str, model_label) {
  
  # Create models for this subscale
  models_sub <- map(1:MasterImp_subset$m, ~ {
    fixest::feols(
      as.formula(paste(outcome_var, formula_str)),
      data = complete(MasterImp_subset, action = .x),
      cluster = ~PatientID
    )
  })
  
  # Pool parameters and add labels
  parameters::pool_parameters(models_sub) |>
    mutate(
      Model = model_label,
      Subscale = subscale_labels[which(faos_subscales == outcome_var)]
    )
...
The incorporation of patient-reported outcome measures into routine clinical foot and ankle orthopaedic practice holds a great deal of promise for better serving patients with appropriate treatments for the broad range of conditions that present for review.

The FAOS [@roos2001] is a 42-question patient-reported outcome measure tailored to general foot and ankle conditions and one of a few scores available for foot and ankle in which its measurement properties have been evaluated according to COSMIN-criteria [@palmen2025].

A clinical quality registry provides an integrated, prospective system to collect and collate outcomes and contextual information regarding foot and ankle orthopaedic conditions, as well as treatments offered at a scale the affords analysis with sufficient sample sizes. However, it presents challenges with respect to the logistics of collecting curated batteries of PROMs at relevant followups. In this present context, all patients are considered part of one group (cohort) and are presented with the same battery of questionnaires at initial review, regardless of the condition. In turn, this poses problems for analysis, in that patient cohorts (groups of patients with similar pathologies and expectations of recovery in both velocity and magnitude) must be generated retrospectively and change in scores before and after treatment interpreted appropriately.

A common use of PROMs is in aid of prognostication of patients at the time treatment is offered. To do this accurately requires an understanding of the factors that confound the relationship between treatment options and observed outcomes and the relevant followup period, as well as the patient's baseline pain, function and quality of life. In a foot-ankle population presenting for surgical review, two key factors of interest that may influence the profile of a patient at baseline are *anatomical region* affected by pathology and whether the pathologies the patient presents with include any form of *arthritis*.

The contemporary literature provides a dearth of information regarding these relationships and even less at sufficient scale to provide reliable guidance to clinicians interested in developing prognostication with PROMs. To address this gap in the knowlege, the current analysis aims to describe baseline pain, function and quality of life in a broad range of orthopaedic conditions, and to assess the influence of *anatomical region* and *arthritis* on these outcomes.

## RECORD \[3\] Objectives

<!--# State specific objectives, including any prespecified hypotheses -->

The questions for the study were outlined explicity in PICO format [@mauffrey2013].
...
Clinical and statistical hypotheses are presented in tabular format.
...
The registry snapshot was processed by retrieving the file and using *tidyverse* to add columns, recode existing columns and create an additional identifier using *tidyr*, representing the patient and their side, to track multiple treatments for each limb. Dates were reformatted to a form appropriate for analysis using *lubridate*. The snapshot is initially processed using custom functions to retrieve datatables and apply eligibility criteria for each treatment
...
Participants were eligible for inclusion in the SOFARI registry if they meet the following criteria
...
The *consort* package was used to create a table of indications for exclusion for each record and plot a flowchart (see RECORD 13.1) indicating the flow from total records processed to the final inclusion sample for analysis.

<!--# going to impute region + pathology columns as required (drop exclusion3) ~ 1400 cases -->

```{r}
#| label: strobeinput-recode

# Fix up treatmentstatus
# 
# 

STROBEFollowUp <- STROBEInput |> filter(
  stringr::str_detect(TreatmentStatus, "follow")
) |> mutate(
  TreatmentStatus = case_when(
    stringr::str_detect(str_to_lower(TreatmentStatusNotes), 
    "(no mention)|refer.*ed|else|((no|not) (intervent|treat))|(enough info)|withdraw|non.*registry|duplicate|confirmation") ~ "Archived",
    is.na(DateTreatment) ~ "Archived",
    is.na(TreatmentStatusNotes) ~ "Ongoing",
    .default = TreatmentStatus
) 
) |> select(all_of(names(STROBEInput)))


STROBEInput2 <- STROBEInput |> rows_update(
  STROBEFollowUp, by = "TreatmentID"
  )
...
STROBEFlow <- STROBEInput2 |> dplyr::filter(
  !is.na(TreatmentID)
) |> left_join(
...
exclusion2 = case_when( #exclusion after induction
    ...
    is.na(exclusion1) & RecordNum > 1 ~ "Subsequent Presentation",
...
The algorithms described for identifying sample selection were validated by manual review with small subsets of data and iterative development of the coded rules to accurately identify exclusion conditions and handle edge cases.
...
Pain catastrophising scores [@cheng2019] derived from the short form version were isolated for further processing.
...
According to [@cheng2019] the PCS-SF (and perhaps the full scale as well) would be best scored by simply adding all the items up. The present analysis may provide a platform for future examination of subscales of PCS such as *Rumination*, *Magnification* and *Helplessness* with respect to baseline pain. The PCS-SF total score was added to the master table.
...
The FAOS scores [@roos2001], including subscores for all 5 dimensions was isolated for further processing.
...
Raw text stored in the registry snapshot was processed with custom functions (see *Functions for Processing*). Abbreviated terminology was isolated and replaced with fully specified versions. Phrases were separated by common delimiters and further processed before being reconstituted into cleaner strings for regular expression matching. Additional processing was performed to create a *Region* label, describing the anatomical region in which the pathology affected.
...
Forefoot - any condition affecting up to the distal heads of the metatarsals

Midfoot - any condition affecting the metatarsals, cuneiforms, navicular and cuboids, as well as soft tissues interacting directly with these bony structures.

Hindfoot - any condition isolated to the calcaneus and talus not involving the talocrural (ankle) joint.

Ankle - conditions affecting the complex bounded by the talus, distal tibial and distal fibula, as well as the soft tissues interacting directly with these bony structures, not isolated to the talus (see hindfoot).

Multiple - presentations involving one or more pathologies that directly affect, or are located within, more than one anatomical region

Foot - conditions that cannot be isolated to a single region of the foot-ankle (e.g. neuropathy or complex pain)
...
Establishing the breakdown of the sample by cohorts revealed that multiple pathologies across multiple regions of the foot-ankle was the most common presentation, followed by ankle_injury and multiple pathologies affecting the ankle (@fig-cohort).
...
The bilateral status was joined back into the master table.
...
Codes and algorithms have been included throughout the report.
...
+-------------------+--------------------------------------------------------------------------------------------------------------+---------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Confounder        | A variable of interest and a target outcome simultaneously influenced by a third variable                    | [@tennant2020]            | models incorporated adjustment for confounders                                                                                                                                                |
+-------------------+--------------------------------------------------------------------------------------------------------------+---------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Descriptives for exposure and outcome variables were summarised into tables compared between anatomical regions.
...
| Sex                    |        | ✅                           |
| Age at surgery         |        | ✅                           |
| Comorbidities          |        | ✅ (SRCQ Score)              |
| Functional status      |        | ✅                           |
| Smoking history        |        | ✅                           |
| Mental state           |        | ✅ (VR12 MCS)                |
| Pain catastrophising   |        | ✅                           |
| Presence of arthritis  |        | ✅                           |
| Bilateral presentation |        | ✅                           |

Summary of potential adjustment set for *FAOS Subscales* against anatomical *region* of pathology retrieved from the literature.
:::

A brief literature review was conducted to identify potential factors that should be adjusted for when assessing the effect of *Region* and *Arthritis* on *FAOS subcales* in the present sample (@tbl-adjustset).

```{r}
#| label: fig-dag-adjust-1
#| fig-cap: "Directed acyclic graph of baseline FAOS against anatomical region in foot and ankle pathology"

DAGFAOS1 <- ggdag::dagify(FAOS ~ Region + Arthritis + Age + Sex + ComorbidityScore + Smoking + Bilateral + MentalScore + PainCatastrophise,
          Region ~ Age + Sex + Smoking + ComorbidityScore,
          Arthritis ~ Bilateral + Age + Sex + Smoking,
          Smoking ~ Age + Sex,
          ComorbidityScore ~ Age + Sex + Smoking,
          Bilateral ~ Age + Smoking,
          MentalScore ~ Age + Sex + Smoking + ComorbidityScore + Arthritis,
          PainCatastrophise ~ Age + Sex,
  labels = c(
    "FAOS" = "FAOS",
...
tidy_dag1 <- ggdag::tidy_dagitty(DAGFAOS1, layout = "auto") |> 
  mutate(label = case_when(
    name == "FAOS" ~ "FAOS",
    name == "SmokingStatus" ~ "Smoking History",
    name == "ComorbidityScore" ~ "Comorbidities",
    name == "Age" ~ "Age at\n Review",
    name == "Region" ~ "AnatomicalRegion",
    name == "MentalScore" ~ "Mental",
    name == "Bilateral" ~ "Bilateral Surgery",
    name == "PainCatastrophise" ~ "PainCatastr",
    name == "Sex" ~ "Sex",
    TRUE ~ name
  ))
...
knitr::knit_print(dagraw)

```

```{r}
#| label: fig-dag-minadjust
#| fig-cap: "Calculated minimum adjustment set for modelling FAOS against anatomical region in foot-ankle pathology"

dagmin <- ggdag_adjustment_set(
  tidy_dag1,
  text = FALSE,
  #use_labels = "label"
  exposure = "Region",
...
knitr::knit_print(dagmin)

```

A minimum adjustment set was derived @fig-dag-minadjust and identified age, sex and comorbidities burden as key adjusters required to assess the effect of anatomical region of baseline FAOS score.
...
MasterAnalysis <- MasterTable6 |> dplyr::select(
  TreatmentID,
  PatientID,
  TreatmentType,
  Sex2,
  ...
  Region,
  starts_with("FAOS")
) |> mutate( #Fix up pathology, add bilateralpath
  SmokingStatus = forcats::fct(
    SmokingStatus_Preop,
    levels = c(
      "Never smoked",
  ...
  Arthritis = if_else(
    stringr::str_detect(Term3,"arthrit"),
    "Yes",
    "No"
  )
) |> dplyr::select(
...
Missingness was visualised with the *naniar* package. It showed that patients with a diagnosis also had other missing data, predominantly PROMs data.
...
```{r}
# Drop variables due to excessive missingness
# 
# 

MasterAnalysis1 <- MasterAnalysis |>
  mutate(
    SubsetGroup_temp = case_when(
Region == "Ankle" & Arthritis == "Yes" ~ "AnkleArthritis",
Region == "Multiple" & Arthritis == "Yes" ~ "MultipleArthritis",
Region == "Ankle" & Arthritis == "No" ~ "AnkleNonArthritis",
.default = NA_character_
),  # Use forcats to create factors with strict level control
  SubsetGroup = fct_relevel(factor(SubsetGroup_temp), "AnkleArthritis", "AnkleNonArthritis","MultipleArthritis"),
  SmokingStatus = if_else(is.na(SmokingStatus), NA_character_, SmokingStatus),
  SmokingStatus = forcats::fct(
    SmokingStatus,
    levels = c("Never smoked","Non smoker for >12 months","Non smoker for <12 months","Yes - current smoker")
           ),
  Sex2 = forcats::fct(
    Sex2,
  ...
  Region = forcats::fct(
    Region,
    levels = c("Ankle","Foot","Multiple","Hindfoot","Midfoot","Forefoot")
    ),
  Provider = forcats::fct(
    Provider,
    levels = c("AW","MS2","MS1","RM")
    ),
  SurgicalTreatment = forcats::fct(
    SurgicalTreatment,
    levels = c("Non-Surgical","Surgical")
    ),
  Arthritis = forcats::fct(
    Arthritis,
    levels = c("No","Yes")
  ),
  BilateralStatus = forcats::fct(
    BilateralStatus,
...
The *mice* package was used to impute the dataset. A predictor matrix was created to remove record identifiers and passive variables (e.g. PCS categorisation) from the imputation. Imputation was performed over 20 replications and stored in a variable for further processing.

```{r}
# Check unique values for each categorical variable
categorical_vars <- c("Sex2", "Provider", "BilateralStatus", "SurgicalTreatment", 
                     "Region", "SmokingStatus", "Arthritis")
...
MasterAnalysis1 |>
  ...
  mutate(
    has_variation = unique_non_missing > 1,
    problem = case_when(
      unique_non_missing <= 1 ~ "No variation in non-missing values",
      missing_count == total_count ~ "All values missing",
      TRUE ~ "OK"
    )
  )

# Also check the actual values
epoxy("Checking actual values for problematic variables:")
for(var in categorical_vars) {
  cat("\n", var, ":\n")
  print(table(MasterAnalysis1[[var]], useNA = "always"))
}
```

```{r}

predM <- mice::make.predictorMatrix(MasterAnalysis1)
...
meth <- mice::make.method(MasterAnalysis1)

# Create visit sequence
visitSeq <- mice::make.visitSequence(MasterAnalysis1)
...
meth["Provider"] <- ""            # No missing values, don't impute
meth["BilateralStatus"] <- ""     # No missing values, don't impute

# Passive imputation for derived variables - use fct_relevel to ensure consistent levels
meth["SubsetGroup"] <- "~I(forcats::fct_relevel(factor(case_when(
Region == `Ankle` & Arthritis == `Yes` ~ `AnkleArthritis`,
Region == `Multiple` & Arthritis == `Yes` ~`MultipleArthritis`,
Region == `Ankle` & Arthritis == `No` ~ `AnkleNonArthritis`
)), 'AnkleArthritis', 'AnkleNonArthritis',`MultipleArthritis`))"
...
A strip plot was used to visually inspect the convergence of the imputation iterations against the original dataset.

```{r}
#| label: fig-stripplot
#| fig-cap: "Strip plot of imputed data over 10 iterations with 20 imputations per iteration."
#| eval: false
...
The estimated FAOS subscores from the imputation model versus the distribution of the observed data were plotted as a visual inspection of the imputation result.

```{r}
#| label: fig-visualise
#| fig-cap: "Estimated versus observed FAOS subcales for the analysis cohort."
...
FAOSSympImp <- ggplot() +
  geom_density(data = complete(MasterImp, 1), aes(x = FAOS_Symptom_TotalScore_Preop, color = "Observed")) +
  geom_density(data = complete(MasterImp, 2:10), aes(x = FAOS_Symptom_TotalScore_Preop, color = "Imputed"))

FAOSDailyImp <- ggplot() +
  geom_density(data = complete(MasterImp, 1), aes(x = FAOS_DailyLiving_TotalScore_Preop, color = "Observed")) +
  geom_density(data = complete(MasterImp, 2:10), aes(x = FAOS_DailyLiving_TotalScore_Preop, color = "Imputed"))

FAOSSportImp <- ggplot() +
  geom_density(data = complete(MasterImp, 1), aes(x = FAOS_Sport_TotalScore_Preop, color = "Observed")) +
  geom_density(data = complete(MasterImp, 2:10), aes(x = FAOS_Sport_TotalScore_Preop, color = "Imputed"))

FAOSQualityImp <- ggplot() +
  geom_density(data = complete(MasterImp, 1), aes(x = FAOS_Quality_TotalScore_Preop, color = "Observed")) +
  geom_density(data = complete(MasterImp, 2:10), aes(x = FAOS_Quality_TotalScore_Preop, color = "Imputed"))

(FAOSPainImp + FAOSSympImp + FAOSDailyImp + FAOSSportImp + FAOSQualityImp) + patchwork::plot_layout(ncol = 2)

```

The match between imputed and observed distributions @fig-visualise, showed reasonable agreement for all subscales, although the Quality of Life subscale displayed some variance that was difficult to match completely in the imputed data.
...
A flow chart was created with the *consort* package to describe the inclusion and exclusion of records into the sample pool for the present analysis to be drawn from. Patient demographics and pathology characteristics were summarised using *gtsummary*. Graphs were generated using *ggplot2*. Summary graphs and tables were prepared for display using *knitr*.

The linear model was applied in *unadjusted* form (Region as the only predictor) and *adjusted* with robust standard errors using the *fixest* package with the form as per the minimum dataset @fig-dag-minadjust;

::: {#regression}
FAOS_Subscale \~ Region + Age + Sex + ComorbidityScore
:::

The adjusted regression was then applied to all FAOS subscales in one function call (see Functions for Processing).

```{r}
#| label: model-analysis
#| code-summary: "Functions to fit fixest models with clustered standard errors for unadjusted and adjusted Region models"
#| message: false

# Define FAOS subscales
faos_subscales <- c("FAOS_Pain_TotalScore_Preop", 
                    "FAOS_DailyLiving_TotalScore_Preop", 
                    "FAOS_Symptom_TotalScore_Preop", 
                    "FAOS_Sport_TotalScore_Preop", 
                    "FAOS_Quality_TotalScore_Preop")

subscale_labels <- c("Pain", "Daily Living", "Symptoms", "Sport", "Quality of Life")

# Create unadjusted models for all subscales
unadjusted_results <- map_dfr(faos_subscales, ~ {
  run_subscale_models(.x, "~ Region", paste("Unadjusted -", subscale_labels[which(faos_subscales == .x)]))
})

# Create adjusted models for all subscales  
adjusted_results <- map_dfr(faos_subscales, ~ {
  run_subscale_models(.x, "~ Region + AgeAtInitialExam + Sex2 + SRCQTotalScore", 
                     paste("Adjusted -", subscale_labels[which(faos_subscales == .x)]))
})

```

An additional function was written to perform pairwise comparisons between *Region* for both unadjusted and adjusted models simultaneously (see Functions for Processing). The function pooled coefficients across multiply imputed regression models. The results were then organised in tabular form and a correction applied for multiple comparisons using the *false discovery method* (FDR) [@benjamini1995]. The results were tabulated for display using the *gt* package.
...
The initial export from the registry returned {nrow(STROBEInput)} treatment records of all types. 
...
```{r}
#| label: fig-strobe
#| fig-cap: "Flow chart of treatment record inclusion for analysis."
...
The flowchart @fig-strobe summarised the retrieval and categorisation of records from the SOFARI registry into the final analysis cohort.
...
Summarising the sample by anatomical region @fig-cohort-breakdown showed the ankle presentations *not* involving arthritis to be most common, followed by cases presenting with pathologies affecting forefoot and multiple regions.
...
The demographics of the cohort @tbl-demographics broken down by anatomical region showed variation between groups for age, sex, bilateral presentation, general health, onset to presentation duration, as well as pain catastrophising.
...
```{r}
#| label: tbl-faos-sum
#| tbl-cap: "Summary of FAOS subscales at baseline assessment of foot and ankle pathology presenting for surgical review"

#Summarise outcome data
#
#

TableFAOS <- gtsummary::tbl_summary(
  MasterAnalysis1,
  include = c(contains("FAOS")),
 missing = "no",
 statistic = list(everything() ~ "{mean} ({sd})")
)

knitr::knit_print(TableFAOS)

```

The summary of baseline FAOS using complete case analysis is presented in @tbl-faos-sum, spanning scores from the highest in Daily Living and the lowest for Quality of Life.

```{r}
#| label: fig-corr-plot
#| fig-cap: "Correlation matrix for FAOS subscales at baseline using imputed data"
#| message: false



# Function to calculate correlations for each imputed dataset
calc_faos_corr <- function(imp_data) {
  faos_data <- imp_data |>
    dplyr::select(contains("FAOS")) |>
    dplyr::rename(
  ...
  #return(cor(faos_data, use = "complete.obs"))
}

# Calculate correlations for each imputed dataset
if (class(MasterImp) == "mids") {
  # If MasterImp is a mids object (from mice)
  corr_list <- map(1:MasterImp$m, function(i) {
    imp_data <- complete(MasterImp, i)
    calc_faos_corr(imp_data)
  })

  # Pool the correlation matrices (average across imputations)
  FAOSCorr <- Reduce("+", corr_list) / length(corr_list)

  # Alternative: Use Rubin's rules for pooling (more complex but more accurate)
  # This would require additional functions for pooling correlations

} else {
  # If MasterImp is already a completed imputed dataset
  FAOSCorr <- calc_faos_corr(MasterImp)
}

# Create the correlation plot
FAOSCorrPlot <- corrplot(
  FAOSCorr, 
  method = "number",
  type = "upper",
  #title = "FAOS Subscales Correlation (Imputed Data)",
  mar = c(0,0,2,0),
  tl.col = "black",
  tl.srt = 45
)
...
The correlation matrix @fig-corr-plot displayed moderate to strong correlations between subscales.
...
The first question involved the effect of anatomical region on variations in FAOS subscales.

```{r}
#| label: tbl-FAOS-unadjust
#| tbl-cap: "Summary of regression assessing the effect of region on FAOS subscales unadjusted for covariates"


# Create unadjusted stacked table
unadjusted_table <- unadjusted_results |>
  ...
  fmt_number(
    columns = c(Coefficient, SE, CI_low, CI_high, df_error),
    decimals = 2
  ) |>
  fmt_number(
    columns = c(p),
  ...
  cols_hide(columns = "Subscale")


knitr::knit_print(unadjusted_table)
```

The unadjusted regression results @tbl-FAOS-unadjust revealed significant differences between Regions for all FAOS subscales, with the largest differences between the reference level (Ankle) and Forefoot.

```{r}
#| label: tbl-FAOS-adjust
#| tbl-cap: "Summary of regression assessing the effect of region on FAOS adjusted for confounders"


# Create adjusted stacked table
adjusted_table <- adjusted_results |>
  gt(groupname_col = "Model") |>
  tab_header(
    title = "Adjusted Regression Results",
    subtitle = "FAOS Subscales ~ Region + Age + Sex + SRCQ Total Score"
  ) |>
  fmt_number(
    columns = c(Statistic),
    decimals = 1
...
knitr::knit_print(adjusted_table)
```

The adjusted regression results @tbl-FAOS-adjust displayed identical patterns to the unadjusted, with the adjustments serving to reduce the optimism by shrinking the coefficients for Forefoot for example across all of the subscales.

```{r}
#| label: tbl-faos-pwc1
#| tbl-cap: "Pairwise comparisons of regional differences in FAOS subscales based on adjusted models"
#| warning: false
#| message: false
...
all_pairwise_results <- bind_rows(
  create_pairwise_comparisons(unadjusted_results),
  create_pairwise_comparisons(adjusted_results)
)

# Extract model type from Model column
all_pairwise_results <- all_pairwise_results |>
  mutate(
    model_type = case_when(
      str_detect(Model, "Unadjusted") ~ "Unadjusted",
      str_detect(Model, "Adjusted") ~ "Adjusted",
      TRUE ~ "Unknown"
    )
  )

# Apply FDR correction within each subscale and model type
final_results <- all_pairwise_results |>
  group_by(Subscale, model_type) |>
  mutate(
...
# Filter for significant or near-significant results and prepare for stacked table
stacked_data <- final_results |>
  filter(p.value < 0.06) |>  # Filter for p < 0.06
  select(Subscale, comparison, model_type, estimate, std.error, p.value, p_fdr, significance) |>
  mutate(
    estimate_ci = paste0(
      sprintf("%.2f", estimate), 
      " (", 
      sprintf("%.2f", estimate - 1.96 * std.error), 
      ", ", 
      sprintf("%.2f", estimate + 1.96 * std.error), 
      ")"
    ),
  ...
  select(Subscale, comparison, model_type, estimate_ci, p_value_formatted, p_fdr_formatted, significance) |>
  pivot_wider(
    names_from = model_type,
    values_from = c(estimate_ci, p_value_formatted, p_fdr_formatted, significance),
    names_sep = "_"
  )

# Create single stacked gt table with subscale row groups
combined_table <- stacked_data |>
  arrange(Subscale, comparison) |>  # Sort by subscale then comparison
  gt(groupname_col = "Subscale") |>  # Use Subscale column for row groups
  tab_header(
    title = "Pairwise Regional Comparisons - All FAOS Subscales",
    subtitle = "Pooled results from multiple imputation with FDR correction (p < 0.06)"
  ) |>
  cols_label(
    comparison = "Comparison",
    estimate_ci_Unadjusted = "Estimate (95% CI)",
    p_value_formatted_Unadjusted = "P-value",
    p_fdr_formatted_Unadjusted = "P-value (FDR)",
    significance_Unadjusted = "",
    estimate_ci_Adjusted = "Estimate (95% CI)",
    p_value_formatted_Adjusted = "P-value", 
    p_fdr_formatted_Adjusted = "P-value (FDR)",
    significance_Adjusted = ""
  ) |>
  tab_spanner(
    label = "Unadjusted Model",
    columns = c(estimate_ci_Unadjusted, p_value_formatted_Unadjusted, p_fdr_formatted_Unadjusted, significance_Unadjusted)
  ) |>
  tab_spanner(
    label = "Adjusted Model",
    columns = c(estimate_ci_Adjusted, p_value_formatted_Adjusted, p_fdr_formatted_Adjusted, significance_Adjusted)
  ) |>
  tab_footnote(
    footnote = "Only comparisons with p-value < 0.06 are shown",
    locations = cells_title(groups = "subtitle")
  ) |>
  tab_footnote(
    footnote = "Adjusted model includes Age, Sex, and SRCQ Total Score as covariates",
    locations = cells_column_spanners(spanners = "Adjusted Model")
  ) |>
  tab_footnote(
    footnote = "*** p<0.001, ** p<0.01, * p<0.05 (FDR corrected)",
    locations = cells_column_labels(columns = c(significance_Unadjusted, significance_Adjusted))
  ) |>
  tab_style(
    style = cell_text(weight = "bold"),
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(p_value_formatted_Unadjusted, p_fdr_formatted_Unadjusted, significance_Unadjusted,
                                      p_value_formatted_Adjusted, p_fdr_formatted_Adjusted, significance_Adjusted))
  ) |>
...
The pairwise comparisons for adjusted and unadjusted models @tbl-faos-pwc1 displayed the largest differences for comparisons involving the *Forefoot* compared to other regions.

### Effect of Arthritis

The second question asked whether the presence of *arthritis* systematically affected FAOS subscales.

```{r}
#| label: tbl-FAOS-arthritis
#| tbl-cap: "Summary of regression assessing the effect of arthritis on FAOS adjusted for confounders"
#| warning: false
#| message: false

# Create adjusted models for all subscales
arthritis_results <- map_dfr(faos_subscales, ~ {
  run_subscale_models(.x, "~ Arthritis + AgeAtInitialExam + Sex2 + SmokingStatus", paste("Arthritis_adjusted -", subscale_labels[which(faos_subscales == .x)]))
})

# Create adjusted stacked table
arthritis_table <- arthritis_results |>
  gt(groupname_col = "Model") |>
  fmt_number(
    columns = c(Statistic),
    decimals = 1
...
knitr::knit_print(arthritis_table)
```

The adjusted models for *Athrtitis* @tbl-FAOS-arthritis displayed significant effects for Arthritis, with all but the Symptoms subscale being much lower than the MCID for FAOS (\~ 15 points).

## RECORD \[17\] Other analyses

<!--# Report other analyses done—eg analyses of subgroups and interactions, and sensitivity analyses -->

A subgroup analysis was performed to compare Ankle Arthritis with Ankle pathology not involving arthritis, as well as Multiple region presentations that also included an arthritis diagnosis.

The multiply imputation dataset was subsetted using filters after conversion to long format.

```{r}

# Function to subset a mids object based on criteria
subset_mids <- function(mids_obj, subset_condition) {
  # Get the original data with all imputations
  long_data <- mice::complete(mids_obj, action = "long", include = TRUE)
  
  # Apply subset condition to the original data (imputation 0)
  original_subset_ids <- long_data |>
    filter(.imp == 0) |>
    filter({{ subset_condition }}) |>
    pull(.id)
  
  # Subset all imputations to keep only these IDs
  subset_long <- long_data |>
    filter(.id %in% original_subset_ids)
  
  # Convert back to mids object
  as.mids(subset_long)
}

# Example subset criteria - adjust as needed for your analysis
# This example keeps cases where Region is "Ankle" or "Foot" and Age >= 18
MasterImp_subset <- subset_mids(
  MasterImp, 
  (Region %in% c("Ankle", "Multiple") & Arthritis == "Yes") | (Region == "Ankle" & Arthritis == "No")
)
...
#  Create the comparison groups on pooled data

 #Create adjusted models for all subscales
arthritis_subset <- map_dfr(faos_subscales, ~ {
  run_subset_models(.x, "~ SubsetGroup + AgeAtInitialExam + Sex2 + SmokingStatus", paste("Arthritis_subset -", subscale_labels[which(faos_subscales == .x)]))
})
...
```{r}
#| label: tbl-subset
#| tbl-cap: "Comparison between sub-groups derived from registry cohort for FAOS subscales adjusted for confounders"


# Create adjusted stacked table
subset_table <- arthritis_subset |> dplyr::filter(
  Parameter == "(Intercept)" | stringr::str_detect(Parameter,"Subset")
) |> dplyr::mutate(
  Parameter = case_when(
    stringr::str_detect(Parameter, "NonArthritis") ~ "Ankle pathology without arthritis",
    stringr::str_detect(Parameter, "MultipleArthritis") ~ "Multiple regions with at least one arthritis diagnosis",
    .default = Parameter
  )
) |> gt(groupname_col = "Model") |>
  tab_header(
    title = "Adjusted Regression Results",
    subtitle = "FAOS Subscales ~ Subgroup + Age + Sex + SRCQ Total Score"
  ) |>
  fmt_number(
    columns = c(Statistic),
    decimals = 1
...
knitr::knit_print(subset_table)
```

Significant reductions in FAOS scores could be attributed to *Arthritis* for all subscales @tbl-subset, although Daily Living was not significantly different between Ankle subgroups, with the largest differences observed for the Symptoms subscale.
...
-   When adjusted for potential confounders, cases presenting with isolated forefoot pathology displayed significantly higher baseline FAOS scores compared to other regions and those presenting with pathology spanning multiple regions

-   When adjusted for potential confounders, the presence of *arthritis* significantly reduced all FAOS scores at baseline, except for Activities of Daily Living.

-   FAOS subscales also showed that patients presenting with isolated ankle arthritis showed significantly lower scores for all FAOS subscales, compared to patients with ankle pathology without arthritis and those with multiple pathologies for the foot and ankle in which arthritis was diagnosed in one or more regions.

## RECORD \[19\] Limitations

<!--# Discuss limitations of the study, taking into account sources of potential bias or imprecision. Discuss both direction and magnitude of any potential bias -->

The present findings should be interpreted -within the context of some key limitations;

-   The methods of classifying diagnoses of foot and ankle conditions require further development as applied in the present context. Classification systems for many conditions are specialised and highly specific, for example ankle fractures or lisfranc injuries of the midfoot. However, there are few examples of classifying foot-ankle pathology holistically including the presence of concomitant pathology to other anatomical foot and ankle regions. While specific classification is viable where the presenting pathology is unequivocal, it becomes problematic to implement in a clinical environment where multiple pathologies affecting multiple anatomical regions are present. Further validation is required to standardise classification in these settings.

    -   Further, the classifications applied in the present study may be too broad to enable meaningful interpretation of the results. While a secondary analysis was conducted on a narrower subset, further work is required to analyse baseline patterns of pain, function and quality of life in more specific presentations, that will also require detailed definitions to identify.

-   The patient population represents a relatively limited presentation of trauma, which is a considerable portion of foot-ankle presentations more generally , but may be under-represented in the present cohort. Although fractures were present in the dataset, these tended to be less severe cases that were at least partway through recovery or were suffering from delayed or non-union. In contrast, more severe cases that were operated prior to clinic presentation at the participating surgeon practices were excluded from the analysis as they could not provide baseline data. The results should be interpreted cautiously in other settings where trauma cases make up a large component of the clinical load. Future work should define appropriate analysis in trauma presentations with respect to what an appropriate baseline assessment is in this population.

-   The present cohort displays a relatively high proportion of missing data (\~45% missing), which although imputation was successfully applied, cannot be assumed to fully mitigate the potential for bias in the models derived. The length of the FAOS questionnaire, combined with other baseline measures, may have formed a barrier to completion for a higher proportion of patients than expected. Further, the one-to-one relationship between scores and treatment records, may have contributed to higher dropout amongs bilateral presentations who may have felt that the measures were duplicated across both sides.

-   A considerable portion of the cohort enrolled into the registry dataset presented during the COVID pandemic (2020-2022). It is possible that the presentation patterns to the clinic were affected by local restrictions enacted by health authorities at different times during the height of the pandemic. These included lockdowns, restricted public transport services, as well as social distancing and other prophylactic measures (e.g. masking) in healthcare settings, which may have altered the presentation patterns.
...
The SOFARI registry was planned with the analysis of PROMs in mind, with background efforts made to review relevant potential confounders to PROMs scores during the initial design of the registry. However, the analysis of anatomical region in this context is outside the original design. Therefore, the presence of unmeasured confounders in adjusted models cannot be entirely ruled out. However, there is limited information available in the literature as to what those confounders may be. Nevertheless, the present results should be interpreted cautiously due to considerable gaps in the present knowledge regarding the most appropriate adjustment model.
...
::: {#fig-comparison}
![](Alexandre%20et%20al%202024%20Table%207.png)

Summary of FAOS subscales from [@alexandre2024]
:::

The systematic review results reproduced in @fig-comparison show the possible range of average baseline scores for FAOS across a variety of foot and ankle pathology cohorts. The present averages summarised in @tbl-faos-sum fall well within the range for each subscale.

However, the present results demonstrate clearly that pathologies restricted to the forefeet return scores that are systematically higher than other regions and those with multiple regions affected (which may include the forefoot). Two key takeaway from the present analysis is firstly, that the FAOS for forefoot conditions may suffer from ceiling effects, particularly for relatively minor conditions. Secondly, future efforts of this kind should consider i) collating forefoot presentations into their own separate cohort and ii) utilising a forefoot-specific PROM, that is appropriate for a broad range of conditions, but is contextually valid to patients presenting for surgical review for forefoot pathology and iii) investigation further the patterns of ceiling effects in these subscales.

The differences between foot-ankle regions can be interpreted in the context of clinically important differences. However, there is limited information available regarding the MCID of FAOS, with only a few studies conducted on relatively small samples, representing a narrow range of conditions using a variety of calculation methods. In short, the MCID for FAOS is variable and ranges from \~ 4 points for the *Symptom* subscale to \~11 points for the *Daily Living* subscale [@tapaninaho2022]. The forefoot differences exceeded these thresholds for all subscales. However, the effect of arthritis was less pronounced and generally failed to exceed MCID thresholds for all subscales.

Where arthritis is present in a clinical presentation of a foot-ankle condition, either as the primary driver of symptoms or concomitant with other pathologies, a patient can be expected to score lower on all FAOS subscales except for Activities of Daily Living. While this effect is not in itself overly surprising, the variation in the magnitude of differences from one subscale to the next is somewhat contrary to popular belief. The variation in differences between those with ankle arthritis, ankle conditions without arthritis present, and patients presenting with multiple pathologies of which arthritis is one of them could be attributed to a number of mechanisms. The first is that patients are unable to differentiate sufficiently for different patterns of pain or function deficits from one condition to the next. The second is that the subscales themselves are not sufficiently discerning to describe patterns of symptoms.

## RECORD \[21\] Generalisability

<!--# Discuss the generalisability (external validity) of the study results -->

The present study represents a relatively large-scale analysis of FAOS subscales across a broad range of conditions electing to undergo surgical review during and some years after the COVID pandemic, within a relatively narrow geographical area of a major metropolitan centre in Australia, with a majority non-urgent case load combined with some trauma cases.

With the COVID pandemic and the mitigation strategies employed exceptional with respect to their effect on healthcare (shift to telemedicine, restriction on theatre access), there is likely to be a finite limit on the generalisability of the present results, particularly as time goes on and case-mix evolves.

Nevertheless, the present study represents one of the largest analyses of foot-ankle pathology in a surgical setting, and describes differences in self-reported pain, function and quality of life with a general scale that has been widely used internationally. Further work should attempt to replicate these findings over a wider collaborative network that includes variation in geography and patient case-mix.
...
The SOFARI registry was self-funded by the participating surgeons. The analysis for this study was also self-funded by the participating surgeons.
