...
The contemporary literature provides a dearth of information regarding these relationships and even less at sufficient scale to provide reliable guidance to clinicians interested in developing prognostication with PROMs.

To address this gap in the knowlege, the current analysis aims to describe baseline pain, function and quality of life in a broad range of orthopaedic conditions, and to assess the influence of *anatomical region* and *arthritis* on these outcomes.
...
Clinical and statistical hypotheses for question 1 and 2. μ₁ᵢ and μ₂ᵢ represent the adjusted mean scores for subscale i in the specific region group and other regions group, respectively.
:::
...
MasterAnalysis1 <- MasterAnalysis |> dplyr::select(
  -(c(
  TreatmentID,
  PreviousPath,
  PreviousSurgery,
...
MasterAnalysis2 <- MasterAnalysis1 |> mutate(
...
for(var in categorical_vars) {
  cat("\n", var, ":\n")
  print(table(MasterAnalysis2[[var]], useNA = "always"))
}
```
...
predM <- mice::make.predictorMatrix(MasterAnalysis2)
...
meth <- mice::make.method(MasterAnalysis2)

# Create visit sequence
visitSeq <- mice::make.visitSequence(MasterAnalysis2)
...
# Set empty methods for variables that shouldn't be imputed
# meth["Sex2"] <- "logreg"          # Binary: Female/Male
# meth["SurgicalTreatment"] <- "logreg"  # Binary: Non-Surgical/Surgical  
# meth["Region"] <- "polyreg"       # Multiple categories: Ankle, Foot, etc.
# meth["SmokingStatus"] <- "polyreg"  # Multiple categories: Never, <12mo, etc.
# meth["Arthritis"] <- "logreg"     # Binary: No/Yes
...
# Convert lapply to map
modelTest1 <- map(1:MasterImp$m, ~ {
  fixest::feols(
...
modelTest2 <- map(1:MasterImp$m, ~ {
  fixest::feols(
    FAOS_Pain_TotalScore_Preop ~ Region + AgeAtInitialExam + Sex2 + SRCQTotalScore,
    data = complete(MasterImp, action = .x),
    cluster = ~PatientID
  )
})
...
```{r}
#| label: subgroup-summary

#  Pool the multiple imputations first
# Pool imputations using mice::complete() with action = "long" for easy handling
pooled_subset <- mice::complete(MasterImp, action = "long", include = TRUE) |>
  # Calculate pooled estimates for each row across all imputations
  group_by(.id) |>
  summarise(
    # Keep categorical/character variables from first imputation
    ...
    #BilateralStatus = first(BilateralStatus),
    Arthritis = first(Arthritis),
    
    # Pool continuous variables by taking the mean across imputations
    AgeAtInitialExam = mean(AgeAtInitialExam, na.rm = TRUE),
    InjuryToPresentation = mean(InjuryToPresentation, na.rm = TRUE),
    VR12_Physical_TotalScore_Preop = mean(VR12_Physical_TotalScore_Preop, na.rm = TRUE),
    VR12_Mental_TotalScore_Preop = mean(VR12_Mental_TotalScore_Preop, na.rm = TRUE),
    SRCQTotalScore = mean(SRCQTotalScore, na.rm = TRUE),
    PCSSF_TotalScore_Preop = mean(PCSSF_TotalScore_Preop, na.rm = TRUE),
    FAOS_Symptom_TotalScore_Preop = mean(FAOS_Symptom_TotalScore_Preop, na.rm = TRUE),
    FAOS_Pain_TotalScore_Preop = mean(FAOS_Pain_TotalScore_Preop, na.rm = TRUE),
    FAOS_DailyLiving_TotalScore_Preop = mean(FAOS_DailyLiving_TotalScore_Preop, na.rm = TRUE),
    FAOS_Sport_TotalScore_Preop = mean(FAOS_Sport_TotalScore_Preop, na.rm = TRUE),
    FAOS_Quality_TotalScore_Preop = mean(FAOS_Quality_TotalScore_Preop, na.rm = TRUE),
    .groups = 'drop'
  )
...
ankle_comparison <- pooled_subset |>
  # Create a comparison group variable
  mutate(
    ComparisonGroup = case_when(
      Region == "Ankle" & Arthritis == "Yes" ~ "Ankle Arthritis",
      Region == "Ankle" & Arthritis == "No" ~ "Ankle Non-Arthritis", 
      Region == "Multiple" ~ "Multiple Regions",
      TRUE ~ "Other"  # This captures all other combinations
    )
  ) |>
  # Filter to keep only the three groups of interest
  filter(ComparisonGroup %in% c("Ankle Arthritis", "Ankle Non-Arthritis", "Multiple Regions"))

```

```{r}
#| label: tbl-subset
#| tbl-cap: "Comparison between sub-groups derived from registry cohort for FAOS subscales and other characteristics"

 # Function to calculate eta-squared for one-way ANOVA (overall effect size)
EtaSquared <- function(data, variable, by, ...) {
  # Perform ANOVA
  model <- aov(data[[variable]] ~ factor(data[[by]]))
  
  # Calculate eta-squared with confidence interval
  es <- effectsize::eta_squared(model, ci = 0.90)
  
  # Format results
  est <- style_sigfig(es$Eta2)
  ci_lower <- style_sigfig(es$CI_low)
  ci_upper <- style_sigfig(es$CI_high)
  
  # Return formatted results
  tibble(
    eta_squared = stringr::str_glue("{est} ({ci_lower}, {ci_upper})"),
    interpret_eta = stringr::str_glue("{effectsize::interpret_eta_squared(es$Eta2, rules = 'field2013')}")
  )
}

# Function to calculate largest raw difference on response scale
MaxRawDifference <- function(data, variable, by, ...) {
  # Calculate group means
  group_means <- data |>
    group_by(!!sym(by)) |>
    summarise(mean_val = mean(!!sym(variable), na.rm = TRUE), .groups = 'drop')
  
  # Find all pairwise differences
  groups <- group_means[[by]]
  means <- group_means$mean_val
  names(means) <- groups
  ...
  for(i in 1:(length(groups)-1)) {
    for(j in (i+1):length(groups)) {
      ...
      pairwise_diffs <- append(pairwise_diffs, diff)
      comparison_names <- append(comparison_names, 
                                paste(groups[i], "vs", groups[j]))
    }
  }
  
  # Find the maximum difference and which groups it's between
  max_diff_idx <- which.max(unlist(pairwise_diffs))
  max_diff <- unlist(pairwise_diffs)[max_diff_idx]
  max_comparison <- unlist(comparison_names)[max_diff_idx]
  
  # Format results
  diff_formatted <- style_sigfig(max_diff)
  
  tibble(
    max_raw_diff = diff_formatted,
    comparison_groups = max_comparison
  )
}

# Step 3: Summary statistics using gtsummary on pooled data
subset_summary <- ankle_comparison |>
  select(ComparisonGroup, AgeAtInitialExam, Sex2,
         VR12_Physical_TotalScore_Preop, VR12_Mental_TotalScore_Preop,
         FAOS_Symptom_TotalScore_Preop, FAOS_Pain_TotalScore_Preop, 
         FAOS_DailyLiving_TotalScore_Preop, FAOS_Sport_TotalScore_Preop, 
         FAOS_Quality_TotalScore_Preop, SRCQTotalScore, PCSSF_TotalScore_Preop) |>
  tbl_summary(
    by = ComparisonGroup,
    statistic = list(
      all_continuous() ~ "{mean} ({sd})",
    ...
    label = list(
      ...
      Sex2 ~ "Sex",
      VR12_Physical_TotalScore_Preop ~ "VR-12 Physical Score",
      VR12_Mental_TotalScore_Preop ~ "VR-12 Mental Score",
      FAOS_Symptom_TotalScore_Preop ~ "FAOS Symptom Score",
      FAOS_Pain_TotalScore_Preop ~ "FAOS Pain Score",
      FAOS_DailyLiving_TotalScore_Preop ~ "FAOS Daily Living Score",
      FAOS_Sport_TotalScore_Preop ~ "FAOS Sport Score",
      FAOS_Quality_TotalScore_Preop ~ "FAOS Quality of Life Score",
      SRCQTotalScore ~ "SRCQ Total Score",
      PCSSF_TotalScore_Preop ~ "PCS-SF Total Score"
    ),
    missing = "no"
  ) |>
  add_n() |>
  ...
  add_stat(fns = all_continuous() ~ EtaSquared) |>
  add_stat(fns = all_continuous() ~ MaxRawDifference) |>
  add_overall() |>
  modify_header(
    label ~ "**Characteristic**",
    eta_squared ~ "**η² (90% CI)**",
    interpret_eta ~ "**η² Interpretation**",
    max_raw_diff ~ "**Largest Difference**",
    comparison_groups ~ "**Between Groups**"
  ) |>
  modify_spanning_header(c("stat_1", "stat_2", "stat_3") ~ "**Group**") |>
  bold_labels()

knitr::knit_print(subset_summary)
```
