...
fit_faos_models <- function(imputed_data, 
                           ...
                           full_covariates = c("Arthritis", "AgeAtInitialExam", "Sex2", 
                                             "BilateralPath", "SRCQTotalScore", "SmokingStatus","VR12_Mental_TotalScore_Preop", "PCSSF_TotalScore_Preop"),
                           interaction_terms = c("Region * Arthritis")) {
...
# Build formula as string (not formula object) to avoid environment issues
  build_formula_string <- function(outcome, type, covariate = NULL) {
    switch(type,
           ...
           "fully_adjusted" = {
             all_terms <- c(full_covariates, interaction_terms)
...
STROBESub <- STROBEInput |> dplyr::select(
 ...
 TreatmentStatus
  ...
  #       stringr::str_detect(str_to_lower(TreatmentStatusNotes), "(sa.*patient)|(sa//ak)|(other.*surg*)|(surgeon.*other)") ~ "Non-Registry Clinician",
  ...
  #   stringr::str_detect(str_to_lower(TreatmentStatusNotes), "intervention|elsewhere|(treated prior)|(no.*(treatment|pathology))|refer*|(no.*diagnosis)|2nd|second|hesitant|(not.*viable)|(failed.*return)|public") ~ "No Treatment Offered",
...
+-------------+-----------------------------------------+----------------------------------------------------------------------------+---------------+
|             | FAOS                                    | 42-question with 5 subscales (Pain, ADL, Sport, Symptoms, Quality of Life) | [@roos2001]   |
+-------------+-----------------------------------------+----------------------------------------------------------------------------+---------------+
...
```{r}
#| label: fig-visualise
#| fig-cap: "Estimated versus observed FAOS-Pain for the analysis cohort."
