...
fit_faos_models <- function(imputed_data, 
  ...
  if(robust_se) {
    if(!require(estimatr)) stop("estimatr package required for robust standard errors")
  ...
  for(outcome in outcomes) {
    ...
    if(robust_se && !is.null(cluster_var)) {
      # Fit with clustered robust standard errors using estimatr
      mira_fit <- with(imputed_data, 
                       lm_robust(as.formula(formula_string), 
                                clusters = get(cluster_var)))
    } else {
