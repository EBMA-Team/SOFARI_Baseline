  ...
  process_chunk <- function(chunk_df) {
    ...
    chunk_df |> 
      ...
      mutate(
        ...
        Hindfoot = if_else(str_detect(chunk_df$Term2, "\\btarsal\\b|\\btalar(?!\\s+dome)|talus|talonavic*|plantar|rearfoot|hindfoot|trigonum|hindfeet|tarsi|calcaneus|\\bcalcaneal\\b|heel|subtalar"),1,0),
...
.default = NA_character_
  ...
  Region = forcats::fct(
    Region,
    levels = c("Ankle","Foot","Multiple","Rearfoot","Midfoot","Forefoot")
    ),
...
The systematic review results reproduced in @fig-comparison show the possible range of average baseline scores for FAOS across a variety of foot and ankle pathology cohorts. The present averages summarised \[table reference\] fall well within the range for each subscale.
