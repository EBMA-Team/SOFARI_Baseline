...
A flow chart was created with the *consort* package to describe the inclusion and exclusion of records into the sample pool for the present analysis to be drawn from. Patient demographics and pathology characteristics were summarised using *gtsummary*. Graphs were generated using *ggplot2*. Graphs and tables were prepared for display using *knitr*. A linear model was fitted to the imputed data with robust standard errors using the *fixest* package with the following form.
...
The models were summarised using *parameters* and *gt* packages. Predicted values for all model variables were generated from the model object using *marginaleffects*.
...
Summarising the sample by anatomical region showed the ankle to be most common presentation, followed by cases presenting with pathologies affecting multiple regions.
...
Table 4: Summary of regression assessing the effect of region on FAOS-Pain adjusted for confounders.
...
create_pairwise_comparisons <- function(pooled_results) {
  ...
  reference_region <- setdiff(
    ...
    c("Ankle", "Foot", "Hindfoot", "Midfoot"), # Replace with your actual regions
    regions
  )[1]
  
  # Create all pairwise combinations
  all_regions <- c(reference_region, regions)
  pairwise_combinations <- combn(all_regions, 2, simplify = FALSE)
...
Review how isolated ankle arthritis compares to other ankle pathology and arthritis spanning multiple foot-ankle regions
