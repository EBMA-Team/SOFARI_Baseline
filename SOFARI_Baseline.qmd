...
fit_faos_models <- function(imputed_data, 
                           ...
                           cluster_var = "patient_id",  # Parameter for clustering variable
                           robust_se = TRUE             # Default to robust SE for population-average effects
                           ) {
  
  model_type <- match.arg(model_type)
  
  # Load required libraries for robust standard errors
  if(robust_se) {
    if(!require(sandwich)) stop("sandwich package required for robust standard errors")
    if(!require(lmtest)) stop("lmtest package required for robust standard errors")
    if(!require(mice)) stop("mice package required for multiply imputed data")
  }
  ...
  build_formula_string <- function(outcome, type, covariate = NULL) {
    switch(type,
           ...
           "fully_adjusted" = {
             all_terms <- c(predictor, full_covariates)
             paste(outcome, "~", paste(all_terms, collapse = " + "))
           },
           "arthritis_adjusted" = {
             all_terms <- c(arthritis_predictor, arthritis_covariates)
             paste(outcome, "~", paste(all_terms, collapse = " + "))
           }
    )
  }
  
  # Function to apply robust SE to a single model fit  
  apply_robust_to_fit <- function(model_fit, cluster_data, cluster_var) {
    robust_coeftest <- coeftest(model_fit, vcov = vcovCL(model_fit, cluster = cluster_data[[cluster_var]]))
    list(
      model = model_fit,
      robust_summary = robust_coeftest,
      robust_confint = confint(robust_coeftest)
    )
  }
  ...
  for(outcome in outcomes) {
    # Extract outcome name and standardize to match expected labels
    outcome_name <- gsub("FAOS_(.+)_TotalScore_Preop", "\\1", outcome)
    
    # Standardize names to match your table function expectations
    outcome_name <- case_when(
      outcome_name == "Pain" ~ "Pain",
      outcome_name == "Symptom" ~ "Symptom", 
      outcome_name == "DailyLiving" ~ "DailyLiving",
      outcome_name == "Sport" ~ "Sport",
      outcome_name == "Quality" ~ "Quality",
      TRUE ~ outcome_name  # fallback
    )
    
    formula_string <- build_formula_string(outcome, model_type, covariate)
    
    # Fit standard linear model using mice::with()
    mira_fit <- with(imputed_data, lm(as.formula(formula_string)))
    
    if(robust_se && !is.null(cluster_var)) {
      # For robust SE, return the mira object directly so it works with pool_and_tidy_mice
      # The robust SE will need to be handled differently with multiple imputation
      model_results[[outcome_name]] <- mira_fit
    } else {
      model_results[[outcome_name]] <- mira_fit
    }
  }
...
# Function to extract robust pooled results (simplified approach)
extract_robust_pooled <- function(robust_results, outcome_name) {
  if(!"robust_fits" %in% names(robust_results[[outcome_name]])) {
    stop("No robust fits found. Make sure robust_se = TRUE was used.")
  }
  
  robust_fits <- robust_results[[outcome_name]]$robust_fits
  
  # Extract coefficients and SEs from each imputation
  coef_list <- lapply(robust_fits, function(x) coef(x$robust_summary))
  se_list <- lapply(robust_fits, function(x) x$robust_summary[, "Std. Error"])
  
  # Simple pooling (you might want to use proper Rubin's rules)
  pooled_coef <- Reduce("+", coef_list) / length(coef_list)
  pooled_se <- sqrt(Reduce("+", lapply(se_list, function(x) x^2)) / length(se_list))
  
  # Create summary similar to pool() output
  result <- data.frame(
    term = names(pooled_coef),
    estimate = pooled_coef,
    std.error = pooled_se,
    statistic = pooled_coef / pooled_se,
    p.value = 2 * pt(abs(pooled_coef / pooled_se), df = Inf, lower.tail = FALSE)
  )
  
  return(result)
}

# Alternative approach: More flexible function that can fit multiple model types at once
fit_faos_models_multi <- function(imputed_data, 
                                 outcomes = c("FAOS_Pain_TotalScore_Preop", 
                                            "FAOS_Symptom_TotalScore_Preop",
                                            "FAOS_DailyLiving_TotalScore_Preop", 
                                            "FAOS_Sport_TotalScore_Preop",
                                 ...
                                 model_configs = list(
                                   fully_adjusted = list(
                                     predictor = "Region",
                                     covariates = c("AgeAtInitialExam", "Sex2", 
                                                   "SRCQTotalScore", "SmokingStatus")
                                   ),
                                   arthritis_adjusted = list(
                                     predictor = "Arthritis", 
                                     covariates = c("Region", "AgeAtInitialExam", "Sex2", "SmokingStatus")
                                   )
                                 ),
                                 cluster_var = "patient_id",  # Parameter for clustering variable
                                 robust_se = TRUE             # Default to robust SE for population-average effects
  ...
  # Build formula string for a given configuration
  build_formula_string <- function(outcome, config) {
    all_terms <- unique(c(config$predictor, config$covariates))
    paste(outcome, "~", paste(all_terms, collapse = " + "))
  }
  ...
  # Fit models for all outcomes and configurations
  all_results <- list()
  ...
  for(config_name in names(model_configs)) {
    ...
    for(outcome in outcomes) {
      ...
      formula_string <- build_formula_string(outcome, config)
...
```{r}
#| label: run-analysis-models

unadjusted_models <- fit_faos_models(MasterImp, model_type = "unadjusted")

adjusted_models <- fit_faos_models(
  MasterImp, 
  model_type = "fully_adjusted", 
  cluster_var = "PatientID"
  )
arthritis_models <- fit_faos_models(
  MasterImp, 
...
adjusted_region_plots <- create_faos_plots(
  adjusted_models,
  plot_type = "region_distribution",
  subscale_names = c("Pain", "Symptoms", "Daily", "Sport", "Quality"))
...
#| tbl-cap: "Pairwise comparisons between regions for model-predicted averages of FAOS subscales"
...
arthritis_table <- create_arthritis_table(arthritis_models)
...
subset_summary <- ankle_comparison |>
  select(ComparisonGroup, AgeAtInitialExam, Sex2, BilateralPres,
  ...
  tbl_summary(
    ...
    label = list(
      AgeAtInitialExam ~ "Age at Initial Exam",
      Sex2 ~ "Sex",
      BilateralPres ~ "Bilateral Presentation",
