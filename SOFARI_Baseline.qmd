...
FAOSPainMod2 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Pain_TotalScore_Preop ~ Region + AgeAtInitialExam + Sex2 + BilateralPres + SRCQTotalScore + VR12_Physical_TotalScore_Preop + VR12_Mental_TotalScore_Preop + PCSSF_TotalScore_Preop
    )
)
...
FAOSSympMod2 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Symptom_TotalScore_Preop ~ Region + AgeAtInitialExam + Sex2 + BilateralPres + SRCQTotalScore + VR12_Physical_TotalScore_Preop + VR12_Mental_TotalScore_Preop
    )
)
...
car::Manova(ManovaMod1, test.statistic = "Pillai")
...
FAOSPlot1 <- ggplot(data = MasterAnalysis, mapping = aes(y = Region, x = FAOS_Pain_TotalScore_Preop)) + stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7) +
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA) +
  scale_fill_brewer(palette = "Set2")

knitr::knit_print(FAOSPlot1)
```
...
ManovaPoolstats <- purrr::map_dbl(ManovaMod1$analyses, ~{
...
miceadds::micombine.F(ManovaPoolstats, df = 4)
```
