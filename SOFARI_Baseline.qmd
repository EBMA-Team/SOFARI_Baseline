...
create_descriptive_table <- function(descriptive_models, 
  ...
  covariate_labels <- switch(covariate_focus,
    ...
    "VR12_Mental_TotalScore_Preop" = list(VR12_Mental_TotalScore_Preop ~ "VR12 Mental Component",
                                         AgeAtInitialExam ~ "Age at Initial Exam", Sex2 ~ "Sex")
  )
...
# Get predictions
PredictGrid1 <- marginaleffects::predictions(FAOSPainMod2)



# Plot
FAOSlmFig1a <- ggplot(PredictGrid1 |> dplyr::filter(AgeAtInitialExam > 15), aes(x = AgeAtInitialExam, y = estimate, 
                                        color = Sex2, fill = Sex2)) +
  geom_point(alpha = 0.1) +  # Show the individual predictions
  geom_smooth(method = "loess", se = TRUE) +  # Smooth line with confidence interval
  scale_fill_brewer(palette = "Set2") +
  scale_color_brewer(palette = "Dark2") +
  labs(
    color = "Sex",
    fill = "Sex",
    y = "Predicted FAOS Pain Score",
    x = "Age at Initial Exam",
    title = "Predicted FAOS Pain Scores by Age and Sex"
  ) +
  theme_minimal()



knitr::knit_print(FAOSlmFig1a)

```

```{r}
# Simple approach - create a list of your models
models <- list(
  Pain = FAOSPainMod2,
  Symptoms = FAOSSympMod2,  # Replace with actual names
  ADL = FAOSADLMod2,
  Sport = FAOSSportMod2,
  QOL = FAOSQOLMod2
)
...
plots1 <- map2(models, names(models), function(model, subscale_name) {

  pred_grid <- marginaleffects::predictions(model)

  ggplot(pred_grid |> filter(AgeAtInitialExam > 15),
         aes(x = AgeAtInitialExam, y = estimate, color = Sex2, fill = Sex2)) +
    geom_point(alpha = 0.1) +
    geom_smooth(method = "loess", se = TRUE) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_brewer(palette = "Dark2") +
    labs(
      ...
      y = paste("Predicted FAOS", subscale_name, "Score"),
      x = "Age at Initial Exam",
      #title = paste("FAOS", subscale_name, "Scores by Age and Sex")
    ) +
    theme_minimal()
})
#
#
# # Create all plots at once
plots2 <- map2(models, names(models), function(model, subscale_name) {

  pred_grid <- marginaleffects::predictions(model)

  ggplot(pred_grid |> filter(AgeAtInitialExam > 15),
         aes(x = SRCQTotalScore, y = estimate, color = Sex2, fill = Sex2)) +
    geom_point(alpha = 0.1) +
    geom_smooth(method = "loess", se = TRUE) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_brewer(palette = "Dark2") +
    labs(
      color = "Sex", fill = "Sex",
      y = paste("Predicted FAOS", subscale_name, "Score"),
      x = "Self-Reported Comorbidities Score",
      #title = paste("FAOS", subscale_name, "Scores by Age and Sex")
    ) +
    theme_minimal()
})
...
#
# # Create all plots at once
plots3 <- map2(models, names(models), function(model, subscale_name) {

  pred_grid <- marginaleffects::predictions(model)

  ggplot(pred_grid |> filter(AgeAtInitialExam > 15),
         aes(x = VR12_Mental_TotalScore_Preop, y = estimate, color = Sex2, fill = Sex2)) +
    geom_point(alpha = 0.1) +
    geom_smooth(method = "loess", se = TRUE) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_brewer(palette = "Dark2") +
...
#
# # Create all plots at once
plots4 <- map2(models, names(models), function(model, subscale_name) {

  pred_grid <- marginaleffects::predictions(model)

  ggplot(pred_grid |> filter(AgeAtInitialExam > 15),
         aes(x = AgeAtInitialExam, y = estimate, color = Arthritis, fill = Arthritis)) +
    geom_point(alpha = 0.1) +
    geom_smooth(method = "loess", se = TRUE) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_brewer(palette = "Dark2") +
    labs(
      color = "Arthritis", fill = "Arthritis",
      y = paste(subscale_name, "Score"),
      x = "Age at Initial Exam",
      #title = paste("FAOS", subscale_name, "Scores by Age and Sex")
    ) +
    theme_minimal()
...
# Create all plots at once
plots5 <- map2(models, names(models), function(model, subscale_name) {

  pred_grid <- marginaleffects::predictions(model)

  ggplot(data = pred_grid, mapping = aes(x = Region, y = estimate)) +
    stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7, fill = "steelblue") +
    stat_dotsinterval(side = "right", scale = 0.7, slab_linewidth = NA, fill = "steelblue") +
    labs(
      y = subscale_name,
...
(plots5$Pain + plots5$Symptoms) / (plots5$ADL + plots5$QOL)

```

```{r}

(plots1$Pain + plots1$Symptoms) / (plots1$ADL + plots1$QOL)

```

```{r}
(plots2$Pain + plots2$Symptoms) / (plots2$ADL + plots2$QOL)
```

```{r}
(plots3$Pain + plots3$Symptoms) / (plots3$ADL + plots3$QOL)
```

```{r}
(plots4$Pain + plots4$Symptoms) / (plots4$ADL + plots4$QOL)
```
...
age_plots$Pain + age_plots$Quality
...
comorbidity_plots$Pain + comorbidity_plots$Quality
...
# Method 1: Using pooled correlations across all imputations
# This is the most statistically appropriate approach
...
adjusted_region_plots$Pain + adjusted_region_plots$Quality
...
create_faos_comparisons <- function(model_list, 
  ...
  # Capture parameters to avoid scoping issues
  var_name <- variable
  ...
  if(combined) {
    ...
    all_comparisons <- map2_dfr(model_list, names(model_list), function(model, subscale_name) {
      ...
      # Create the variables list for avg_comparisons
      variables_list <- setNames(list(comp_type), var_name)
    ...
    result <- all_comparisons |>
      ...
      fmt_number(columns = c(estimate, std.error, p.value), decimals = dec) |>
    ...
    comparison_tables <- map2(model_list, names(model_list), function(model, subscale_name) {
      ...
      comp_table <- comparisons |>
        dplyr::select(contrast, estimate, std.error, p.value) |>
        gt() |>
        fmt_number(decimals = dec) |>
...
combined_comparisons <- create_faos_comparisons(adjusted_models, combined = TRUE)
...
Filter for ankle arthritis and plot against other characteristics

```{r}
#| label: tbl-demographics2
#| tbl-cap: "Summary of patient and treatment record characteristics for the ankle arthritis sub-sample."

MasterAnalysisSub <- MasterTable5 |> dplyr::select(
  TreatmentID:Satisfaction_Preop,
  Pathology,
  Region,
  BilateralPres,
...
TblDemoSub <- gtsummary::tbl_summary(
  MasterAnalysisSub |> dplyr::select(
    !(c(
      TreatmentID,
      PatientID,
      Pathology,
...
label = list(
    ...
    Sex2 ~ "Female",
    Satisfaction_Preop ~ "Symptom Satisfaction",
    PCSSF_TotalScore_Preop ~ "Pan Catastrophising"
    ),
    type = list(
    Sex2 ~ "dichotomous"
    ),
...
knitr::knit_print(TblDemoSub)


```

```{r}
#| label: fig-simple-model1
#| fig-cap: "Relationship between interaction effect of sex and age with FAOS-Pain at baseline"

FAOSPlotAA <- ggplot(data = MasterAnalysisSub |> dplyr::filter(!is.na(Sex2)), mapping = aes(y = FAOS_Pain_TotalScore_Preop, x = Sex2)) + stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7, fill = "steelblue") +
...
FAOSlmAA <- lm(
    ...
    data = MasterAnalysisSub
)

# Create the expanded grid manually
prediction_dataAA <- tidyr::expand_grid(
  Sex2 = unique(MasterAnalysisSub$Sex2[!is.na(MasterAnalysisSub$Sex2)]),
  AgeAtInitialExam = seq(min(MasterAnalysisSub$AgeAtInitialExam, na.rm = TRUE),
                         max(MasterAnalysisSub$AgeAtInitialExam, na.rm = TRUE),
                         length.out = 101)
)
# 
  FAOSlmFigSub <- augment(FAOSlmAA, newdata = prediction_dataAA, se_fit = TRUE) |>
  ggplot(aes(x = AgeAtInitialExam, fill = ordered(Sex2), color = ordered(Sex2))) +
  stat_lineribbon(
    aes(ydist = distributional::dist_student_t(df = df.residual(FAOSlmAA), mu = .fitted, sigma = .se.fit)),
    alpha = 1/4
  ) +
  geom_point(aes(y = FAOS_Pain_TotalScore_Preop), data = MasterAnalysisSub) +
  scale_fill_brewer(palette = "Set2") +
  scale_color_brewer(palette = "Dark2") +
  labs(
    color = "Sex2",
