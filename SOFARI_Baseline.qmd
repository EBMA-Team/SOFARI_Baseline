...
fit_faos_models <- function(imputed_data, 
                           ...
                           cluster_var = "PatientID",  # Parameter for clustering variable
  ...
  for(outcome in outcomes) {
    ...
    if(robust_se && !is.null(cluster_var)) {
      # Store both original model and robust results
      robust_coeftest <- coeftest(base_model, vcov = vcovCL(base_model, cluster = imputed_data[[cluster_var]]))
      
      model_results[[outcome_name]] <- list(
        model = base_model,
        robust_summary = robust_coeftest,
        robust_confint = confint(robust_coeftest)
      )
    } else {
      model_results[[outcome_name]] <- base_model
    }
  }
...
adjusted_models <- fit_faos_models(MasterImp, model_type = "unadjusted")
...
::: {#fig-comparison}
![](Alexandre et al 2024 Table 7.png)
