...
tokenize_diagnosis <- memoise::memoise(function(df, stop_words = NULL, batch_size = 500, process_batch) {
  ...
  df <- df |> dplyr::mutate(global_id = row_number())
...
```{r}
#| label: categorise-diagnosis-func
...
```{r}
#| label: concatenate-diagnoses-func
...
```{r}
#| label: concatenate-cumulative-func
...
categorize_pathology <- function(df, config) {
  ...
  df |> 
    mutate(
      ...
      # Final classifications
      Other = as.numeric(str_detect(term_replaced, "foreign.+(body|material)")),
      NegatePathology = as.numeric(str_detect(term_replaced, "(?<!ab?)normal|(?<!(non|mal)-?)|nil.+pathology|\\bheal\\b|reduced|non-tender|unremarkable", negate = TRUE))
    )
}
```
...
concatenate_results <- function(df) {
  ...
  sequence_level <- df |> 
    ...
    summarise(
      ...
      PathologySum = max(PathologySum, na.rm = TRUE),
      AnatomySum = max(AnatomySum, na.rm = TRUE),
      .groups = "keep"
    ) |> 
...
# Step 1: Add classification logic after creating DiagTerm
add_classification_logic <- function(diag_term_df) {
  
  # Step 1: Create the classification flags (like your old DiagFunc2)
  diag_classified <- diag_term_df |>
    mutate(
      # Calculate classification flags
      has_region = rowSums(across(Ankle:Foot), na.rm = TRUE),
      has_pathology = rowSums(across(Arthritis:Instability), na.rm = TRUE),
      RowCount1 = n(),
      .by = TreatmentID
    ) |>
    mutate(
  ...
  # Step 2: Split classified from unclassified (like DiagFunc2a/2b/2c)
  classified_terms <- diag_classified |>
    filter(Product > 0, ResetFlag == 0)
  
  # Get unclassified terms and concatenate them
  unclassified_base <- diag_classified |>
    filter(ResetFlag == 1) |>
    distinct(TreatmentID, .keep_all = TRUE)
  
  unclassified_concatenated <- diag_classified |>
    filter(ResetFlag == 1) |>
    group_by(TreatmentID) |>
    summarise(
      # Concatenate the combined terms
      CombinedTerm_new = str_c(CombinedTerm, collapse = "; "),
      # Aggregate anatomy columns (take max)
      Ankle = max(Ankle, na.rm = TRUE),
      Rearfoot = max(Rearfoot, na.rm = TRUE),
      Midfoot = max(Midfoot, na.rm = TRUE),
      Forefoot = max(Forefoot, na.rm = TRUE),
      Foot = max(Foot, na.rm = TRUE),
      # Aggregate pathology columns (take max)
      Arthritis = max(Arthritis, na.rm = TRUE),
      Injury = max(Injury, na.rm = TRUE),
      Deformity = max(Deformity, na.rm = TRUE),
      Metatarsalgia = max(Metatarsalgia, na.rm = TRUE),
      Soft_tissue_disorder = max(Soft_tissue_disorder, na.rm = TRUE),
      Growth = max(Growth, na.rm = TRUE),
      Neural = max(Neural, na.rm = TRUE),
      Infection = max(Infection, na.rm = TRUE),
      Impingement = max(Impingement, na.rm = TRUE),
      Instability = max(Instability, na.rm = TRUE),
      .groups = "drop"
    ) |>
    # Recalculate sums
    mutate(
      PathologySum = rowSums(across(Arthritis:Instability), na.rm = TRUE),
      AnatomySum = rowSums(across(Ankle:Foot), na.rm = TRUE),
      Other = if_else(PathologySum > 0 | AnatomySum > 0, 1, 0),
      NegatePathology = 0  # You may need to recalculate this
    )
  
  # Join the concatenated terms back to the base
  unclassified_final <- unclassified_base |>
    select(-CombinedTerm, -(Ankle:NegatePathology)) |>
    left_join(unclassified_concatenated, by = "TreatmentID") |>
    rename(CombinedTerm = CombinedTerm_new)
  
  # Step 3: Combine classified and unclassified terms back together
  result <- bind_rows(classified_terms, unclassified_final) |>
    arrange(TreatmentID) |>
    select(-(has_region:ResetFlag)) |>
    # Recreate the cumulative diagnosis
    group_by(TreatmentID) |>
    mutate(
      CumulativeDiagnosis = str_c(CombinedTerm, collapse = "; "),
      SequenceRow = row_number()
    ) |>
    ungroup()
...
# Step 2: Apply the classification logic to your DiagTerm
DiagTerm_with_classification <- add_classification_logic(DiagTerm)

# Step 3: Filter to only keep classified terms (like your old pipeline did)
DiagTerm_final <- DiagTerm_with_classification |>
  mutate(
    has_region = rowSums(across(Ankle:Foot), na.rm = TRUE),
    has_pathology = rowSums(across(Arthritis:Instability), na.rm = TRUE)
  ) |>
  filter(has_region * has_pathology >= 1) |>
  # Update sequence ID
  mutate(
    SequenceIDUpdate = paste(TreatmentID, SequenceRow, sep = "."),
    .by = TreatmentID
  ) |>
  select(-has_region, -has_pathology)

# Step 4: Now create your region and pathology summaries
DiagRegion_new <- DiagTerm_final |>
  select(CombinedTerm, SequenceIDUpdate, TreatmentID, Ankle:Foot) |>
  pivot_longer(
    cols = !c(TreatmentID, SequenceIDUpdate, CombinedTerm),
    names_to = "Region", 
    values_to = "RegionPresence"
  ) |>
  filter(RegionPresence > 0, !is.na(Region)) |>
  mutate(
    RegionCount = n_distinct(Region),
    RegionCat = if_else(RegionCount > 1, "Multiple", "Isolated"),
    .by = TreatmentID
  ) |>
  select(-RegionPresence)

DiagPathology_new <- DiagTerm_final |>
  select(CombinedTerm, TreatmentID, SequenceIDUpdate, Arthritis:Other) |>
  pivot_longer(
    cols = !c(TreatmentID, SequenceIDUpdate, CombinedTerm),
    names_to = "Pathology",
    values_to = "PathPresence"  
  ) |>
  filter(PathPresence > 0, !is.na(Pathology)) |>
  mutate(
...
# Step 5: Create the distinct summaries
DiagRegion1_new <- DiagRegion_new |>
  distinct(TreatmentID, .keep_all = TRUE)

DiagPath1_new <- DiagPathology_new |>
  distinct(TreatmentID, .keep_all = TRUE)
```
...
```{r}
#| label: faos-model-func

# Main modelling function
fit_faos_models <- function(imputed_data, 
                           ...
                           predictor = "Region",
                           model_type = c("minimal", "unadjusted", "fully_adjusted"),
                           covariate = NULL,  # For minimal models - which covariate to focus on
...
# Build formula as string (not formula object) to avoid environment issues
  build_formula_string <- function(outcome, type, covariate = NULL) {
    switch(type,
           "minimal" = {
             if(is.null(covariate)) stop("covariate must be specified for minimal models")
             adjusters <- setdiff(minimal_adjusters, covariate)  # Don't adjust for the covariate itself
             if(length(adjusters) > 0) {
               paste(outcome, "~", covariate, "+", paste(adjusters, collapse = " + "))
             } else {
               paste(outcome, "~", covariate)
             }
           },
           "unadjusted" = paste(outcome, "~", predictor),
           "fully_adjusted" = {
             all_terms <- c(full_covariates)
  ...
  for(outcome in outcomes) {
    outcome_name <- gsub("FAOS_(.+)_TotalScore_Preop", "\\1", outcome)
    
    formula_string <- build_formula_string(outcome, model_type, covariate)
    
    # Convert to formula inside the with() call to ensure correct environment
    model_results[[outcome_name]] <- with(
      imputed_data,
      exp = lm(as.formula(formula_string))
    )
  }
...
create_faos_regression_table <- function(model_list, 
                                        table_type = c("unadjusted", "minimally_adjusted", "fully_adjusted"),
                                        custom_labels = NULL,
                                        estimate_digits = 2,
                                        pvalue_digits = 3) {
  ...
  default_labels <- switch(table_type,
    ...
    "fully_adjusted" = list(
      ...
      # AgeAtInitialExam ~ "Age at Initial Exam", 
      # Sex2 ~ "Sex",
      # BilateralPath ~ "Bilateral Pathology",
      # SRCQTotalScore ~ "Self-Reported Comorbidities",
      # VR12_Mental_TotalScore_Preop ~ "VR12 Mental Component",
      # PCSSF_TotalScore_Preop ~ "Pain Catastrophizing Scale"
    )
  )
...
create_unadjusted_table <- function(unadjusted_models, 
                                   table_caption = "Summary of regression assessing the effect of region on FAOS (unadjusted)") {
  
  table <- create_faos_regression_table(unadjusted_models, table_type = "unadjusted")
  
  # Add caption if in Quarto/RMarkdown context
...
create_descriptive_table <- function(descriptive_models, 
  ...
  covariate_labels <- switch(covariate_focus,
    ...
    "VR12_Mental_TotalScore_Preop" = list(VR12_Mental_TotalScore_Preop ~ "VR12 Mental", Sex2 ~ "Sex")
  )
  ...
  if(is.null(table_caption)) {
    ...
    table_caption <- paste("FAOS scores by", covariate_name, "(minimally adjusted)")
  }
...
create_adjusted_table <- function(adjusted_models,
                                 table_caption = "Summary of regression assessing the effect of region on FAOS (fully adjusted)") {
...
# Batch function to create all three table types at once
create_all_faos_tables <- function(unadjusted_models, 
                                  descriptive_models_list,  # Named list with covariate as key
                                  adjusted_models) {
  
  # Create unadjusted table
  unadjusted_table <- create_unadjusted_table(unadjusted_models)
  
  # Create descriptive tables for each covariate
  descriptive_tables <- map2(descriptive_models_list, names(descriptive_models_list), 
                            function(models, covariate) {
    create_descriptive_table(models, covariate)
  })
  
  # Create adjusted table  
  adjusted_table <- create_adjusted_table(adjusted_models)
  
  return(list(
    unadjusted = unadjusted_table,
...
```{r}
#| label: model-plot-func

# Plotting function that works with any model type
create_faos_plots <- function(models, 
  ...
  # Create plots
  plots <- map2(models, names(models), function(model, subscale_name) {
    ...
    if(plot_type == "region_distribution") {
      ...
      ggplot(data = pred_grid, mapping = aes(x = !!sym(config$x_var), y = estimate)) +
        ...
        labs(
          y = paste("Predicted FAOS", subscale_name, "Score"),
          x = config$x_label
        ) +
        theme_minimal() +
      ...
      # Standard continuous variable plots
...
```{r}
#| label: wrapper-model-func


# Convenience wrapper functions for your three model types

# 1. Minimally adjusted models for descriptive analysis
fit_descriptive_models <- function(imputed_data, covariate, outcomes = NULL) {
  if(is.null(outcomes)) {
    outcomes <- c("FAOS_Pain_TotalScore_Preop", "FAOS_Symptom_TotalScore_Preop",
                  "FAOS_DailyLiving_TotalScore_Preop", "FAOS_Sport_TotalScore_Preop",
                  "FAOS_Quality_TotalScore_Preop")
  }
  
  fit_faos_models(imputed_data, outcomes = outcomes, model_type = "minimal", covariate = covariate)
}

# 2. Unadjusted predictor models
fit_unadjusted_models <- function(imputed_data, outcomes = NULL) {
  if(is.null(outcomes)) {
    outcomes <- c("FAOS_Pain_TotalScore_Preop", "FAOS_Symptom_TotalScore_Preop",
                  "FAOS_DailyLiving_TotalScore_Preop", "FAOS_Sport_TotalScore_Preop",
                  "FAOS_Quality_TotalScore_Preop")
  ...
  fit_faos_models(imputed_data, outcomes = outcomes, model_type = "unadjusted")
}

# 3. Fully adjusted models (your current approach)
fit_adjusted_models <- function(imputed_data, outcomes = NULL) {
  if(is.null(outcomes)) {
    outcomes <- c("FAOS_Pain_TotalScore_Preop", "FAOS_Symptom_TotalScore_Preop",
                  "FAOS_DailyLiving_TotalScore_Preop", "FAOS_Sport_TotalScore_Preop",
                  "FAOS_Quality_TotalScore_Preop")
  ...
  fit_faos_models(imputed_data, outcomes = outcomes, model_type = "fully_adjusted")
}
```
...
create_faos_comparisons <- function(model_list, 
                                   ...
                                   decimals = 3,
                                   fdr_correction = TRUE,
                                   table_caption = "Pairwise comparisons between regions for model-predicted FAOS scores") {
  ...
  if(combined) {
    ...
    if(fdr_correction) {
      ...
      result <- all_comparisons |>
        ...
        tab_footnote(
          footnote = "FDR correction applied across all 75 comparisons (15 region pairs × 5 FAOS subscales)",
...
```{r}
#| label: batch-process-diagnosis1
...
# Example usage
DiagTerm <- process_clinical_diagnoses(
  data = MasterTable3,
  target_terms_url = TargetURL,
  batch_size = 750,
  use_parallel = TRUE
...
```{r}
DiagTermTreat <- DiagTerm |>
  group_by(TreatmentID) |>
  mutate(
    # Aggregate anatomy columns - use max to get 1 if ANY term has that anatomy
    AnkleTreat  = max(Ankle, na.rm = TRUE),
    RearfootTreat  = max(Rearfoot, na.rm = TRUE), 
    MidfootTreat  = max(Midfoot, na.rm = TRUE),
    ForefootTreat  = max(Forefoot, na.rm = TRUE),
    FootTreat  = max(Foot, na.rm = TRUE),
    
    # Aggregate pathology columns
    ArthritisTreat  = max(Arthritis, na.rm = TRUE),
    InjuryTreat = max(Injury, na.rm = TRUE),
    DeformityTreat  = max(Deformity, na.rm = TRUE),
    MetatarsalgiaTreat  = max(Metatarsalgia, na.rm = TRUE),
    Soft_tissue_disorderTreat  = max(Soft_tissue_disorder, na.rm = TRUE),
    GrowthTreat  = max(Growth, na.rm = TRUE),
    NeuralTreat  = max(Neural, na.rm = TRUE),
    InfectionTreat  = max(Infection, na.rm = TRUE),
    ImpingementTreat  = max(Impingement, na.rm = TRUE),
    InstabilityTreat  = max(Instability, na.rm = TRUE),
    
    # Recalculate sums
    PathologySumTreat  = ArthritisTreat + InjuryTreat + DeformityTreat + MetatarsalgiaTreat + 
                       Soft_tissue_disorderTreat + GrowthTreat + NeuralTreat + InfectionTreat + 
                       ImpingementTreat + InstabilityTreat,
    AnatomySumTreat  = AnkleTreat + RearfootTreat + MidfootTreat + ForefootTreat + FootTreat
  ) |>
  ungroup()
```
...
```{r}
# After creating your DiagTerm from the tokenization pipeline
DiagTerm_final <- add_classification_logic(DiagTermTreat) |> mutate(
  RegionCat = case_when(
          AnatomySum == 0 ~ NA_character_,
          AnatomySum == 1 ~ "Isolated", 
          AnatomySum > 1 ~ "Multiple"
        ),
  PathCat = case_when(
          PathologySum == 0 ~ NA_character_,
          PathologySum == 1 ~ "Isolated",
          PathologySum > 1 ~ "Multiple"  
        )
      )
```
...
MasterTable5 <- MasterTable4 |> left_join(
  DiagTerm_final |>
      filter(SequenceRow == 1) |>  # Get one row per TreatmentID
      dplyr::mutate(
        RegionCat = case_when(
          AnatomySum == 0 ~ NA_character_,
          AnatomySum == 1 ~ "Isolated", 
      ...
      dplyr::select(TreatmentID, RegionCat, PathCat),
    by = "TreatmentID"
  )
...
#| fig-cap: "Directed acyclic graph of baseline FAOS in foot and ankle pathology"

DAGFAOS <- ggdag::dagify(FAOS ~ Region + Arthritis + Age + Sex + ComorbidityScore + Smoking + Bilateral + MentalScore + PainCatastrophise,
...
tidy_dag1 <- ggdag::tidy_dagitty(DAGFAOS, layout = "auto") |> 
...
MasterAnalysis <- MasterTable6 |> dplyr::select(
  ...
  BilateralPath = if_else(
    (BilateralPres == "Unilateral" & stringr::str_detect(Term3,"bilateral")) | BilateralPres != "Unilateral",
    "Bilateral",
    "Unilateral"
  )
) |> dplyr::select(
...
unadjusted_models <- fit_unadjusted_models(MasterImp)
adjusted_models <- fit_adjusted_models(MasterImp)
...
#| fig-cap: "Breakdown of sample by diagnostic group - region labels."
...
label = list(
    ...
    SmokingStatus_Preop ~ "Smoking History"
    ),
    type = list(
    Sex2 ~ "dichotomous"
    ),
...
# Get all completed datasets from your imputed object
completed_datasets <- mice::complete(MasterImp, "all")

# Transform each dataset
transformed_datasets <- map(completed_datasets, ~ {
  .x %>% 
    mutate(
      Provider2 = case_when(
        Provider == "AW" ~ "A",
    ...
    dplyr::select(!(c(TreatmentID, PatientID, Provider)))
})

# Pool the data
pooled_data <- transformed_datasets[[1]]  # Start with first dataset as template

# Get variable names
var_names <- names(pooled_data)

# Pool each variable
for (var in var_names) {
  if (is.numeric(pooled_data[[var]])) {
    # For numeric variables: average across all imputations
    values_matrix <- map_dfc(transformed_datasets, ~ .x[[var]]) 
    pooled_data[[var]] <- rowMeans(values_matrix, na.rm = TRUE)
  } else {
    # For categorical variables: take the most frequent value for each row
    n_rows <- nrow(pooled_data)
    pooled_values <- character(n_rows)
    
    for (row in 1:n_rows) {
      # Get values from all imputations for this row
      row_values <- map_chr(transformed_datasets, ~ as.character(.x[[var]][row]))
      # Find most frequent value
      tab <- table(row_values)
      pooled_values[row] <- names(tab)[which.max(tab)]
    }
    
    # Preserve factor structure if original was factor
    if (is.factor(transformed_datasets[[1]][[var]])) {
      pooled_data[[var]] <- factor(pooled_values, levels = levels(transformed_datasets[[1]][[var]]))
    } else {
      pooled_data[[var]] <- pooled_values
    }
  }
}

# Now create your gtsummary table
TblDemo2 <- gtsummary::tbl_summary(
  pooled_data,
  by = "Region",
  label = list(
    Provider2 ~ "Provider",
...
(age_plots$Pain + age_plots$Symptom + age_plots$Daily + age_plots$Sport + age_plots$Quality) + patchwork::plot_layout(ncol = 2)
...
comorbidity_plots <- create_faos_plots(comorbidity_models, plot_type = "comorbidities_sex")

(comorbidity_plots$Pain + comorbidity_plots$Symptom + comorbidity_plots$Daily + comorbidity_plots$Sport + comorbidity_plots$Quality) + patchwork::plot_layout(ncol = 2)
...
(arthritis_plots$Pain + arthritis_plots$Symptom + arthritis_plots$Daily + arthritis_plots$Sport + arthritis_plots$Quality) + patchwork::plot_layout(ncol = 2)
```
...
#| tbl-cap: "Summary of regression assessing the effect of region on FAOS unadjusted"
...
adjusted_region_plots <- create_faos_plots(adjusted_models, plot_type = "region_distribution")


(adjusted_region_plots$Pain + adjusted_region_plots$Symptom + adjusted_region_plots$Daily + adjusted_region_plots$Sport + adjusted_region_plots$Quality) + patchwork::plot_layout(ncol = 2)

```

```{r}
#| label: tbl-pwc-faospain
#| tbl-cap: "Pairwise comparisons between regions for model-predicted averages of FAOS Pain"


combined_comparisons <- create_faos_comparisons(adjusted_models, combined = TRUE, fdr_correction = TRUE)
knitr::knit_print(combined_comparisons)
...
# Step 1: Pool the multiple imputations first
...
pooled_subset <- mice::complete(MasterImp, action = "long", include = TRUE) %>%
  ...
  summarise(
    ...
    BilateralPres = first(BilateralPres),
    Region = first(Region),
    Arthritis = first(Arthritis),
    BilateralPath = first(BilateralPath),
    Satisfaction_Preop = first(Satisfaction_Preop),
...
```{r}
# Step 2: Create the comparison groups on pooled data
