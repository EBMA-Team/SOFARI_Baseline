...
STROBESub2 <- STROBESub |>
  ...
  dplyr::mutate(
    TreatmentStatusNotes2 = dplyr::case_when(
      ...
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "referred|(referral.*neuropathic)") ~ "Referred elsewhere",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "opt|op-|withdr*") ~ "Opt out",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "non.*registry|pathology|shoulder|wrist|knee|amput*|acl") ~ "Non-registry pathology",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "attend|cancel|cnx|canx|cx|(never came)|arrive|missed|show|(no referral)") ~ "Did not attend",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "((treatme.*t|tx) offered)|mention|menion|(surgery elsewhere)|(no intervention)|(surgery in ed)|hesitant.*treat") ~ "No treatment offered",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "((sa|ak) patient)|(w sa)|(other surgeon)|(joint consult)") ~ "Non-registry surgeon",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "duplicate|accessory|error|reop|complication|incorrect|(additional treatment)|superfluous") ~ "Accessory record",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "pre.*registry|prior|(existing treatment)") ~ "Followup of pre-registry treatment",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "imaging|mri|(returned for results)") ~ "Incomplete diagnosis",
      stringr::str_detect(str_to_lower(TreatmentStatusNotes), "no.*(note|letter|info|record)|insufficient") ~ "Insufficient information",
      .default = NA_character_
    )
  )
```
...
FAOSPainMod1 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Pain_TotalScore_Preop ~ Region*Arthritis
    )
)
...
FAOSPainMod2 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Pain_TotalScore_Preop ~ Region * Arthritis + AgeAtInitialExam + Sex2 + BilateralPath + SRCQTotalScore + VR12_Mental_TotalScore_Preop + PCSSF_TotalScore_Preop
    )
)
...
FAOSSympMod1 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Symptom_TotalScore_Preop ~ Region*Arthritis
    )
)
...
FAOSSympMod2 <- with(
  MasterImp, 
  exp = lm(
    FAOS_Symptom_TotalScore_Preop ~ Region * Arthritis + AgeAtInitialExam + Sex2 + BilateralPath + SRCQTotalScore + VR12_Mental_TotalScore_Preop
    )
)
...
Complete case analysis displayed unique distributions of FAOS - Pain total score between diagnostic groups.

```{r}
#| label: fig-faospain-cca
#| fig-cap: "FAOS Pain subscale Total Score by Diagnostic Group"


FAOSPlot1 <- ggplot(data = MasterAnalysis, mapping = aes(y = FAOS_Pain_TotalScore_Preop, x = Region, fill = Arthritis)) + 
  stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7) +
  stat_dotsinterval(side = "right", scale = 0.7, slab_linewidth = NA) + 
  labs(y = "FAOS Pain") +
  scale_fill_manual(values = c("Yes" = "steelblue", "No" = "lightblue")) +
...
FAOSPlot4 <- ggplot(data = MasterAnalysis |> dplyr::filter(!is.na(Arthritis)), mapping = aes(y = FAOS_Pain_TotalScore_Preop, x = Arthritis)) + stat_slabinterval(aes(thickness = after_stat(pdf*n)), scale = 0.7, fill = "steelblue") +
  stat_dotsinterval(side = "right", scale = 0.7, slab_linewidth = NA, fill = "steelblue") + labs(y = "FAOS Pain")

# Combine the plots
(FAOSPlot1 + FAOSPlot2) / (FAOSPlot3 + FAOSPlot4)
...
FAOSlm1 <- lm(
    FAOS_Pain_TotalScore_Preop ~ Region * Arthritis,
    data = MasterAnalysis |> dplyr::filter(!(Region == "Foot"))
)

# Create the expanded grid manually
prediction_data1 <- tidyr::expand_grid(
  Region = unique(MasterAnalysis$Region[!is.na(MasterAnalysis$Region) & MasterAnalysis$Region != "Foot"]),
  Arthritis = unique(MasterAnalysis$Arthritis[!is.na(MasterAnalysis$Arthritis)])
)

  FAOSlmFig1 <- augment(FAOSlm1, newdata = prediction_data1, se_fit = TRUE) |>
  ggplot(aes(x = Region, fill = ordered(Arthritis), color = ordered(Arthritis))) +
  stat_lineribbon(
...
knitr::knit_print(FAOSlmFig1)

```

```{r}
#| label: fig-simple-model2
#| fig-cap: "Relationship between interaction effect of sex and self-reported comorbidities score with FAOS-Pain at baseline"
...
```{r}
#| label: fig-corr-plot
#| fig-cap: "Correlation matrix for FAOS subscales at baseline"
...
Table 5: Pairwise comparisons between regions for model-predicted averages of FAOS-Pain.
