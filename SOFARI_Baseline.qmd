...
```{r}
#| label: fig-FAOS-adjust
#| tbl-cap: "Adjusted FAOS subscales across anatomical regions adjusted for potential confounders"


# Modified plotting function using pooled parameter results
create_faos_pooled_plots <- function(pooled_results, 
                                     model_type = "adjusted",
                                     show_reference = TRUE,
                                     fixed_y_scale = TRUE,
  ...
  # Filter for the specified model type and region parameters
  plot_data <- pooled_results |>
    filter(str_detect(Model, str_to_title(model_type))) |>
    filter(str_detect(Parameter, "^Region")) |>
    # Clean up region names by removing "Region" prefix
    mutate(
      Region = str_remove(Parameter, "^Region"),
      # Handle reference category if needed
      Region = case_when(
        Region == "" ~ "Ankle",
        TRUE ~ Region
      )
  ...
  # Add reference category (estimate = 0, CI = 0) if requested
  if(show_reference) {
    reference_data <- plot_data |>
      distinct(Subscale) |>
      mutate(
        Parameter = "RegionReference",
        Region = "Ankle", 
        Coefficient = 0,
        SE = 0,
        CI_low = 0,
    ...
    plot_data <- bind_rows(plot_data, reference_data)
  }
  
  # Convert to response scale if requested
  if(response_scale && !is.null(reference_values)) {
    plot_data <- plot_data |>
      mutate(
        ref_value = map_dbl(Subscale, ~reference_values[[.x]] %||% 0),
        Coefficient = Coefficient + ref_value,
        CI_low = CI_low + ref_value,
        CI_high = CI_high + ref_value
      )
  }
  
  # Calculate common y-axis limits if fixed scale is requested
  if(fixed_y_scale) {
    y_min <- min(c(plot_data$CI_low), na.rm = TRUE)
    y_max <- max(c(plot_data$CI_high), na.rm = TRUE)
    y_range <- y_max - y_min
    y_limits <- c(y_min - 0.05 * y_range, y_max + 0.05 * y_range)
  }
  
  # Create individual plots for each subscale using stat_slabinterval
  plots <- plot_data |>
    group_by(Subscale) |>
    group_split() |>
    map(function(subscale_data) {
      
      subscale_name <- unique(subscale_data$Subscale)
      
      # Create expanded data for stat_slabinterval to work with
      # This simulates draws from the posterior distribution
      expanded_data <- subscale_data |>
        rowwise() |>
        mutate(
          # Generate normal distribution samples based on coefficient and SE
          draws = list(rnorm(1000, mean = Coefficient, sd = SE))
        ) |>
        unnest(draws) |>
        select(Region, Subscale, estimate = draws)
      
      # Set y-axis label based on scale
      y_label <- if(response_scale) {
        paste("Predicted", subscale_name, "Score")
      } else {
        paste("Predicted", subscale_name)
      }
      
      p <- ggplot(data = expanded_data, mapping = aes(x = Region, y = estimate)) +
        stat_slabinterval(
      ...
      # Add reference line (0 for coefficient scale, or don't add for response scale)
      if(!response_scale) {
  ...
  # Add names to the plots list
  names(plots) <- plot_data |>
    pull(Subscale) |>
    unique()
...
# Function to combine plots using patchwork
combine_faos_plots <- function(plot_list, 
                               ncol = 3,
                               nrow = 2,
                               title = "FAOS Subscale Regional Effects",
                               subtitle = NULL,
                               caption = NULL) {
...
# Convenience function to create and combine plots in one step
create_and_combine_faos_plots <- function(pooled_results,
                                          model_type = "adjusted",
                                          show_reference = TRUE,
                                          fixed_y_scale = TRUE,
                                          response_scale = FALSE,
                                          ...
                                          nrow = 2,
                                          title = "FAOS Subscale Regional Effects",
                                          subtitle = "Adjusted models with 95% confidence intervals",
                                          caption = "Note: Predictions based on pooled results from multiply imputed data") {
  
  # Create individual plots
  plots <- create_faos_pooled_plots(
  ...
  # Combine using patchwork and return only the combined plot
  combined_plot <- combine_faos_plots(
    plot_list = plots,
    ncol = ncol,
    nrow = nrow,
    title = title,
...
PredictFaos <- create_and_combine_faos_plots(
  pooled_results = adjusted_results,
  model_type = "adjusted"
)


knitr::knit_print(PredictFaos)
```
...
```{r}
# Look at your parameter names
epoxy("=== STEP 1: DATA EXPLORATION ===")
arthritis_params <- arthritis_subset |>
  filter(str_detect(Parameter, "arthritis|subset")) |>
  pull(Parameter) |>
  unique()

epoxy("Parameters found: {paste(arthritis_params, collapse = ', ')}")

# Look at the structure
glimpse(arthritis_subset)
```

```{r}
# Step 2: Clean and identify groups
epoxy("\n=== STEP 2: GROUP IDENTIFICATION ===")

# Extract group information based on your parameter names
group_data <- arthritis_subset |>
  filter(str_detect(Parameter, "^SubsetGroup")) |>
  mutate(
    Group = str_remove(Parameter, "^SubsetGroup")
  ) |>
  select(Subscale, Model, Parameter, Group, Coefficient, SE)

# Check what groups we found
unique_groups <- unique(group_data$Group)
epoxy("Groups found: {paste(unique_groups, collapse = ', ')}")
epoxy("Number of groups: {length(unique_groups)}")
```
```{r}
# Step 3: Define your reference group
epoxy("\n=== STEP 3: REFERENCE GROUP ===")

# Your reference group is the one NOT in the model parameters
# Based on your data, it appears to be "AnkleArthritis" (the reference category)
reference_group <- "AnkleArthritis"

# All groups for comparisons
all_groups <- c(reference_group, unique_groups)
epoxy("All groups for pairwise comparisons: {paste(all_groups, collapse = ', ')}")
epoxy("Total groups: {length(all_groups)}")

# Check we have enough groups
if(length(all_groups) < 2) {
  stop("Need at least 2 groups. Found: ", length(all_groups))
}

epoxy("✓ Ready for pairwise comparisons!")
```

```{r}
# Step 4: Create pairwise combinations
epoxy("\n=== STEP 4: PAIRWISE COMBINATIONS ===")

pairwise_combinations <- combn(all_groups, 2, simplify = FALSE)
combination_names <- map_chr(pairwise_combinations, ~paste(.x[1], "vs", .x[2]))
epoxy("Number of combinations: {length(pairwise_combinations)}")
epoxy("Pairwise comparisons to create:")
for(i in seq_along(combination_names)) {
  epoxy("  {i}. {combination_names[i]}")
}
...
calculate_difference <- function(group1, group2, data_subset) {
  
  epoxy("  Calculating: {group1} vs {group2}")
  
  # Get coefficient for group1 (0 if reference)
  coef1 <- if(group1 == reference_group) {
    0
  } else {
    result <- data_subset |> filter(Group == group1) |> pull(Coefficient)
    if(length(result) == 0) result <- 0
    result
  }
  
  # Get SE for group1 (0 if reference)  
  se1 <- if(group1 == reference_group) {
    0
  } else {
    result <- data_subset |> filter(Group == group1) |> pull(SE)
    if(length(result) == 0) result <- 0
    result
  }
  
  # Same for group2
  coef2 <- if(group2 == reference_group) {
    0
  } else {
    result <- data_subset |> filter(Group == group2) |> pull(Coefficient)
    if(length(result) == 0) result <- 0
    result
  }
  
  se2 <- if(group2 == reference_group) {
    0
  } else {
    result <- data_subset |> filter(Group == group2) |> pull(SE)
    if(length(result) == 0) result <- 0
    result
  }
...
# Apply to each subscale and model
pairwise_results <- group_data |>
  nest(data = c(Group, Coefficient, SE)) |>
  mutate(
    comparisons = map(data, function(subscale_data) {
      map_dfr(pairwise_combinations, function(pair) {
        calculate_difference(pair[1], pair[2], subscale_data)
      })
    })
  ) |>
  select(-data) |>
...
epoxy("Pairwise calculations complete!")
...
subset_results <- pairwise_results |>
  group_by(Subscale, Model) |>
...
subset_results |>
  filter(p.value < 0.06) |>
  select(Subscale, Model, comparison, estimate_ci, p.value, p_fdr)

epoxy("Analysis complete! {nrow(filter(subset_results, p.value < 0.06))} comparisons with p < 0.06")
```
...
# Create formatted GT table from pairwise results
create_pairwise_table <- function(final_results, p_threshold = 0.06) {
  
  # Filter and format the data
  subset_pwc_table <- subset_results |>
    filter(p.value < p_threshold) |>
    mutate(
      # Clean up comparison names for better display
      comparison = str_replace_all(comparison, c(
        "AnkleArthritis" = "Ankle Arthritis",
        "AnkleNonArthritis" = "Ankle Non-Arthritis", 
        "MultipleArthritis" = "Multiple Arthritis"
      )),
      ...
      model_type = case_when(
        str_detect(Model, "Unadjusted|unadjusted") ~ "Unadjusted",
        str_detect(Model, "Adjusted|adjusted") ~ "Adjusted",
        TRUE ~ "Model"
      )
    ) |>
    select(Subscale, comparison, model_type, estimate_ci, p_formatted, p_fdr_formatted) |>
    # Pivot to get model types as columns
    pivot_wider(
      names_from = model_type,
      values_from = c(estimate_ci, p_formatted, p_fdr_formatted),
      names_sep = "_"
    )
  
  # Create the GT table
  subset_pwc_table_gt <- subset_pwc_table |>
    arrange(Subscale, comparison) |>
    gt(groupname_col = "Subscale") |>
    tab_header(
      title = "Pairwise Arthritis Group Comparisons - FAOS Subscales",
      subtitle = paste0("Pooled results from multiple imputation with FDR correction (p < ", p_threshold, ")")
    ) |>
    cols_label(
      comparison = "Comparison",
      estimate_ci_Model = "Estimate (95% CI)",
      p_formatted_Model = "P-value",
      p_fdr_formatted_Model = "P-value (FDR)"
    ) |>
    tab_footnote(
      footnote = paste0("Only comparisons with p-value < ", p_threshold, " are shown"),
      locations = cells_title(groups = "subtitle")
    ) |>
    tab_style(
      style = cell_text(weight = "bold"),
    ...
    tab_style(
      style = cell_text(align = "center"),
      locations = cells_body(columns = c(contains("p_formatted")))
    ) |>
    opt_table_font(font = "Arial") |>
    tab_options(
      table.font.size = 12,
  ...
  # Handle different model types if they exist
  if("estimate_ci_Unadjusted" %in% names(subset_pwc_table)) {
    subset_pwc_table_gt <- subset_pwc_table |>
      cols_label(
        estimate_ci_Unadjusted = "Estimate (95% CI)",
        p_formatted_Unadjusted = "P-value",
        p_fdr_formatted_Unadjusted = "P-value (FDR)",
        significance_Unadjusted = "",
        estimate_ci_Adjusted = "Estimate (95% CI)",
        p_formatted_Adjusted = "P-value", 
        p_fdr_formatted_Adjusted = "P-value (FDR)"
      ) |>
      tab_spanner(
        label = "Unadjusted Model",
        columns = c(estimate_ci_Unadjusted, p_formatted_Unadjusted, p_fdr_formatted_Unadjusted)
      ) |>
      tab_spanner(
        label = "Adjusted Model", 
        columns = c(estimate_ci_Adjusted, p_formatted_Adjusted, p_fdr_formatted_Adjusted)
      ) |>
      tab_footnote(
        footnote = "Adjusted model includes covariates as specified in original analysis",
        locations = cells_column_spanners(spanners = "Adjusted Model")
      )
  }
  
  return(subset_pwc_table_gt)
}

# Create and display the table
subset_pwc_table <- create_pairwise_table(subset_pwc_results)

# Show summary
epoxy("Table created with {nrow(filter(final_results, p.value < 0.06))} significant comparisons")
...
<!--# RECORD 19.1: Discuss the implications of using data that were not created or collected to answer the specific research question(s). Include discussion of misclassification bias, fit_adjusted_modelsunmeasured confounding, missing data, and changing eligibility over time, as they pertain to the study being reported. -->
