...
unadjusted_models <- fit_faos_models_cached(
...
The model was summarised using *gtsummary*. Predicted values for all model variables were generated from the model object using *marginaleffects*.
...
(adjusted_region_plots$Pain + adjusted_region_plots$Symptom + adjusted_region_plots$Daily + adjusted_region_plots$Sport + adjusted_region_plots$Quality) + patchwork::plot_layout(ncol = 3)
